...
The associated version can be assigned with ``[git ref]=[version]`` syntax, with the caveat that the specified version
is known to Spack from either the package definition, or in the configuration preferences (i.e. ``packages.yaml``).
...
An additional syntax requirement for using version assigned git refs is there must be a space after the version if
variants are going to be specified directly after the version.

.. code-block:: sh
   foo@git.feature=develop +mpi # good, will parse
   foo@git.feature=develop+mpi  # bad, will not parse
