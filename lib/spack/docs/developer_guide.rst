...
In a git checkout, only files changed relative to ``--base`` are checked by the import check. If
the repository is not a git checkout, or has no ``--base`` revision (as with the shallow clones
Spack makes of package repositories), Spack warns and checks every file in it instead. Files
given explicitly must be inside the repository.
