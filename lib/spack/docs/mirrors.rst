...
Adding a mirror really adds an entry in the mirrors config, by default stored in the user environment scope at ``~/.config/spack/mirrors.yaml``:
...
The mirror is maintained locally at :file:`$data_home/downloads`.
