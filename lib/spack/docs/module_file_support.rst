...
|          |                   |                         |                                                  | Modules, Lmod    |
+----------+-------------------+-------------------------+--------------------------------------------------+------------------+
| ``lmod`` | hierarchical      | ``share/spack/lmod``    | ``share/spack/templates/modules/modulefile.lua`` | Lmod             |
+----------+-------------------+-------------------------+--------------------------------------------------+------------------+
...
When hierarchical mode is enabled the default projection for module file is ``{name}/{version}`` otherwise it is ``{name}/{version}-{compiler.name}-{compiler.version}``.

Module files can also be organized in a non-hierarchical way.
This is the default when ``tcl`` is activated and it can be enabled for ``lmod`` with ``hierarchical: false``.
