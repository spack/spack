...
Module files generated by Spack expect that only one version of a given package is loaded in the environment at a time.
Lmod enforces this automatically with its "one name rule", but Environment Modules does not, unless its ``unique_name_loaded`` configuration option (introduced in version 5.4) is enabled.
For this reason, tcl module files declare by default a conflict on their own module name: with ``app/1`` and ``app/2`` installed, both module files contain a ``conflict app`` statement, so only one of them can be loaded at a time.
This statement is harmless when the generated tcl module files are used with Lmod, as it is redundant with the "one name rule".

The conflict is derived from the projection in use: it corresponds to the projection truncated after the first path component holding the ``{name}`` token.
For instance, a ``conflict arch/app`` statement results from an ``{architecture}/{name}/{version}`` projection.
If no such conflict can be expressed — for example, with a flat projection like ``{name}-{version}`` — none is added.

Set the ``conflict_on_name`` option to ``false`` to disable this behavior:
...
.. code-block:: yaml
   ...
   modules:
     default:
       tcl:
         all:
           conflict_on_name: false

Additional conflicts between module files can be declared explicitly with the ``conflict`` key under a spec pattern, to prevent two incompatible modules from being loaded at the same time:
...
For Lmod, and for Environment Modules versions prior to 4.2, such a conflict must be expressed on both module files conflicting with each other.
