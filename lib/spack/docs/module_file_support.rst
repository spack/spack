...
If you have a module system in your path you should be able to run either
``module avail`` or ``use -l spack`` to see what module files have been
installed. A sample output of those commands will be something like:
...
at any login you can put the source line in your ``.bashrc`` or ``.cshrc``.
Be aware though that the startup time may be slightly increased because of that.
...
the modules it depends on.  This is not required most of the times
because Spack builds binaries with RPATH support. However, there are a few
relevant exceptions to this scheme: Python extensions, for instance,
are currently *not* built with RPATH.
...
You can activate any other module file generator shipped with Spack by adding it to the
