...
* Added the :func:`~spack.package.deprecated` directive.
  The concretizer raises a concretization error when a selected deprecation's severity exceeds the threshold configured via ``deprecation:allowed_severity`` in :ref:`packages.yaml <package-deprecations-config>`.
