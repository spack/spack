...
Spack's build defaults are in the default ``$spack/etc/spack/defaults/packages.yaml`` file.
You can override them in ``~/.config/spack/packages.yaml`` or ``$spack/etc/spack/packages.yaml``.
