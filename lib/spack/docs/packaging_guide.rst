...
Often packages will need to provide attributes for dependents to query
various details about what it provides.  While any number of custom defined
attributes can be implemented by a pacakage, the specific four attributes
...
=========== =========================================== =====================
``root``    The installation directory for the package  ``spec.prefix``
``command`` An executable command for the package       | ``spec.name`` in
                                                        | ``spec.prefix.bin``
...
.. code-block:: python
   ...
   class Foo(Package):
       ...
       def libs(self):
           ...
           return find_libraries('libMyFoo', root=self.spec.prefix, recursive=True)
...
.. code-block:: python
   ...
   class Foo(Package):
       ...
       @property
       def headers(self):
           return find_headers('foo', root=self.spec.prefix.include, recursive=False)

       # Just the foo libraries
       @property
       def libs(self):
           return find_libraries('libFoo', root=self.spec.prefix, recursive=True)

       # The header provided by the bar virutal package
       @property
       def bar_headers(self):
           return find_headers('bar/bar.h', root=self.spec.prefix.include, recursive=False)

       # The libary provided by the bar virtual package
       @property
       def bar_libs(self):
           return find_libraries('libFooBar', root=sef.spec.prefix, recursive=True)
       ...
       def baz_headers(self):
           return find_headers('baz/baz', root=self.spec['baz'].root.include, recursive=False)

       # The library provided by the baz virtual package
       @property
       def baz_libs(self):
           return find_libraries('libFooBaz', root=self.spec['baz'].root, recursive=True)
...
.. code-block:: python
   ...
   # root defaults to the package install prefix without an explicit implementation
   >>> spec['foo'].root
   '/opt/spack/linux-fedora35-haswell/gcc-11.3.1/foo-1.0-ca3rczp5omy7dfzoqw4p7oc2yh3u7lt6'
...
.. code-block:: python
   ...
   # root defaults to the foo prefix without either a Foo.bar_root
   # or Foo.root implementation
   >>> spec['bar'].root
   '/opt/spack/linux-fedora35-haswell/gcc-11.3.1/foo-1.0-ca3rczp5omy7dfzoqw4p7oc2yh3u7lt6'
...
.. code-block:: python
   ...
   # baz_root implementation provides the subdirectory inside the foo prefix
   >>> spec['baz'].root
...
^^^^^^^^^^^^^^^^^^^^^^^^^^^^
Using ``root`` vs ``prefix``
^^^^^^^^^^^^^^^^^^^^^^^^^^^^

``root`` and ``prefix`` are both attributes that can be queried on a
...
location on disk where a concrete package resides, ``root`` is the `logical`
...
cases inside a package dependency paths should be accessed via
``self.spec['foo'].root`` instead of ``self.spec['foo'].prefix``.

See :ref:`custom-attributes` for more details and an example implementing
a custom ``root`` attribute.
