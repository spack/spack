...
.. code-block:: python
   ...
   class MyConstrainedPackage(Package):
       ...
       @when('@main')
       def test(self):
           # TODO: Add quick checks of installed software for the main version
           pass
...
.. note::
   ...
   If the two test methods for ``MyConstrainedPackage`` are implemented
   and the user installs a version that doesn't match the specs, then no
   tests will be executed by ``spack test run`` and ``spack test results``
   will report the status as ``NO-TESTS``.
...
to the corresponding test suite. You can also specify `--` before
the spec(s) to get all tests that match the specs.
