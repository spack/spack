...
for customizing Spack's behavior with respect to
software installation. 

We first cover the configuration file
hierarchy, then configuration options for compilers,
focusing on extending Spack's compiler auto-detection.

Next, we will discuss the packages configuration file, focusing on
...
For all of these features, we demonstrate how we build up a full
configuration file. For some, we demonstrate how the
configuration affects the install command, and for others, we use
...
four configuration *scopes* to handle this customization. These scopes,
in decreasing priority, are:
...
but should never be edited directly. To override these settings,
...
use the ``$SPACK_ROOT/etc/spack`` directory.

For settings specific to a particular user, add
configuration files to the ``~/.spack`` directory. When Spack first
checked for compilers on your system, it
placed your compiler configuration in this directory.

Some facilities manage multiple platforms from a single shared
file system. To handle this, each of the configuration
...
These files are listed in decreasing precedence, so files in
...
the configuration scopes are updated as dictionaries in 
order of increasing precedence, allowing higher precedence files to override
...
replaces what was in it with the new value. For example, a
...
configuration file had a single colon instead of the double, it
...
circumstances we want even finer control over the
compilers available. You can achieve that using the compilers-configuration file.
...
Let's talk about the sections of this compiler entry that we changed.
The biggest change is to the ``paths`` section. This lists
...
compiler for both specifications of Fortran. We also changed the
...
This sets the default preferences for compilers and providers of
virtual packages. To illustrate how this works, suppose we want to
change the preferences to prefer the Clang compiler and 
MPICH over Open MPI. Currently, we prefer GCC and Open MPI.
...
overrides the default settings for just these two items.
...
packages without shared libraries. We accomplish this by turning
...
need serial HDF5, typing
``hdf5~mpi`` all the time would get annoying quickly. Instead, we'll update our preferences for
HDF5.
...
In general, every attribute that we can set for all packages can be
...
Spack to use the system zlib, we have two choices:
specify it on the command line or tell Spack that it's
...
build with an alternative MPI implementation.
...
we can cut down on the work of specifying
...
you may not be able to build at all with so many cores. On nodes with 64+
...
If we uninstall and reinstall zlib, we see that it now uses only four cores:
