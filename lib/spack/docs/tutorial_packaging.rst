...
This tutorial covers how to build a simple
package installation script.  We focus on building an mpileaks package,
which is a MPI debugging tool.  By creating a package file, we're
essentially giving Spack a recipe for building a particular piece of
software.  We describe some of the software's dependencies, where to
find the package, what commands and options are used to build the package
from source, and more.  Once we've specified the recipe, we can
ask Spack to build the package in many different ways.

This tutorial assumes you are familiarity with some Spack
commands and have a working version of Spack installed.  If
not, we suggest looking at Spack's *Getting Started* guide.  We
tutorial also assumes a beginner-level familiarity
with Python.

This tutorial can help you get started
with packaging, but is not intended as complete documentation.  See Spack's
:ref:`packaging-guide` for more on this topic.
...
- Make sure your ``EDITOR`` environment variable is set to a text
  editor you like.
...
This command takes the location of a package's source code, downloads
the code, and sets up some basic packaging infrastructure for you.  The
mpileaks source code can be found on GitHub. Here's what happens when
...
moment to look it over.  Spack has
created a few placeholders, which we'll fill in during this tutorial.

- We'll document some information about the package in the comments.
- We'll fill in the dependency list.
- We'll fill in some of the configuration arguments needed to build the
  package.
...
It obviously didn't work. We need to fill in some package-specific
...
copyright clause at this point to keep this document shorter,
...
and below.  Make these changes to your ``package.py``:

.. literalinclude:: tutorial/examples/1.package.py
   :lines: 25-
   :language: python

We filled in the comment that describes what this package does and
added a link to the web site.  That won't help us build yet, but will
...
dependencies before building mpileaks.  The mpi dependency is a
different kind of beast from the adept-utils and callpath dependencies;
...
This command may take awhile to run and produce more output if
you don't have an MPI already installed or configured in Spack.

Now Spack has identified our dependencies and made sure they have been
...
you that we never ran the configure script.  Let's add a
...
all brilliant developers and use this opportunity to spend some
time debugging.  We have a few options that can tell us about
what's going wrong.

Per the error message, Spack has given us a ``spack-build.out`` debug log:
...
mpileaks can be picky and still want things spelled out on
their command line.  But let's continue to pretend we're not brilliant
developers, and explore some other debugging paths.

We can enter the build area and try to manually run the build:
...
That's all we need for working mpileaks!  If we install now we'll see:
...
a full install routine, we would have written:
...
To do this, we'll add a variant to our package, per the following (see
...
configure line (with output truncated for length):
...
This tutorial glossed over a few important features that weren't
too relevant for mpileaks, but may be useful to other packages.  There
are several places we reference the ``self.spec`` object.  This is a
...
what version of a package is being installed.  Full documentation is in the :ref:`packaging-guide`, but here are some quick snippets with
common queries:
