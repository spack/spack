...
case "$command" in
    cpp)
        ...
        debug_flags="-g"
        ;;
    cc|c89|c99|gcc|clang|armclang|icc|icx|pgcc|nvc|xlc|xlc_r|fcc|cl.exe|cl.sh)
        command="$SPACK_CC"
        language="C"
        comp="CC"
        lang_flags=C
    ...
    c++|CC|g++|clang++|armclang++|icpc|icpx|pgc++|nvc++|xlc++|xlc++_r|FCC)
        ...
        debug_flags="-g"
        ;;
    ftn|f90|fc|f95|gfortran|flang|armflang|ifort|ifx|pgfortran|nvfortran|xlf90|xlf90_r|nagfor|frt|ifx.sh)
        command="$SPACK_FC"
        language="Fortran 90"
        comp="FC"
        lang_flags=F
