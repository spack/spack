...
MH_FLAGS_DESCRIPTIONS = {
    ...
    MH_ALLMODSBOUND: "indicates that this binary binds to all two-level namespace modules "
    "of its dependent libraries",
    MH_SUBSECTIONS_VIA_SYMBOLS: "safe to divide up the sections into sub-sections via symbols for "
    "dead code stripping",
    ...
    MH_SETUID_SAFE: "the binary declares it is safe for use in processes when issetugid() "
    "is true",
    MH_NO_REEXPORTED_DYLIBS: "the static linker does not need to examine dependent dylibs to see "
    "if any are re-exported",
    MH_PIE: "the OS will load the main executable at a random address",
    MH_DEAD_STRIPPABLE_DYLIB: "the static linker will automatically not create a LC_LOAD_DYLIB load "
    ...
    MH_NO_HEAP_EXECUTION: "the OS will run the main executable with a non-executable heap "
