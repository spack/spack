...
class match_to_ansi:
    def __init__(self, color: bool = True, enclose: bool = False, zsh: bool = False):
    ...
    def __call__(self, match):
        ...
        ansi_code = self.escape(f"{styles[style]};{colors.get(color, '')}")
        colored_text = f"{text}{self.escape(0)}" if text else ""
        return f"{ansi_code}{colored_text}"
...
def colorize(string: str, color: bool = None, enclose: bool = False, zsh: bool = False) -> str:
    ...
    return COLOR_RE.sub(match_to_ansi(color, enclose), string, zsh).replace("}}", "}")
