    ...
    for mirror in binary_mirrors:
        ...
        for layout_version in mirror.supported_layout_versions:
            ...
            cache_entry = cache_class(
                mirror.fetch_url, spec, allow_unsigned=not mirror.signed
            )
...
class IndexHandler:
    ...
    def get_index_manifest(self, manifest_response) -> BlobRecord:
        ...
        try:
            ...
            raise FetchIndexError(f"Remote index {self.url} is invalid", e) from e
...
class SCPIndexHandler(IndexHandler):
    ...
    def conditional_fetch(self) -> FetchIndexResult:
        ...
        try:
            _, _, response = web_util.read_from_url(url_index_manifest)
        except Exception as e:
            raise FetchIndexError(
                f"Could not read index manifest from {url_index_manifest}"
            ) from e
