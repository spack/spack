...
class BinaryTextReplaceException(spack.error.SpackError):
    ...
    def __init__(self, old_path, new_path):
        msg = "New path longer than old path: binary relocation not possible"
...
def relocate_package(spec, allow_root):
    ...
    old_rel_prefix = buildinfo.get('relative_prefix',
                                   'not/in/buildinfo/dictionary')
    old_prefix = old_path + os.sep + old_rel_prefix
    ...
    for path_name in path_names:
        relocate.replace_prefix_text(path_name, old_prefix, new_prefix)
...
def extract_tarball(spec, filename, allow_root=False, unsigned=False,
    ...
    if (old_relative_prefix != new_relative_prefix and (rel or
        ...
        msg += "uses relative rpaths or contains mach-o bins."
