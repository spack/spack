...
class BinaryCacheIndex(object):
    ...
    def _associate_built_specs_with_mirror(self, cache_key, mirror_url):
        ...
        try:
            db_root_dir = tmpdir.joinpath('db_root')
...
def binary_index_location():
    ...
    cache_root = misc_cache_location().joinpath('indices')
...
def build_cache_prefix(prefix):
    return prefix.joinpath(build_cache_relative_path())
...
def buildinfo_file_name(prefix):
    ...
    """
    name = prefix.joinpath(".spack/binary_distribution")
    return name
...
def get_buildfile_manifest(spec):
    ...
    for root, dirs, files in os.walk(spec.prefix, topdown=True):
        ...
        for directory in dirs:
            dir_path_name = root.joinpath(directory)
        ...
        for filename in files:
            path_name = root.joinpath(filename)
...
def _read_specs_and_push_index(file_list, cache_prefix, db, db_root_dir):
    ...
    # the mirror.
    index_json_path = db_root_dir.joinpath('index.json')
    ...
    index_hash_path = db_root_dir.joinpath('index.json.hash')
...
def generate_key_index(key_prefix, tmpdir=None):
    ...
    if keys_local:
        target = keys_local.joinpath('index.json')
    else:
        if not tmpdir:
            tmpdir = tempfile.mkdtemp()
            remove_tmpdir = True
        target = tmpdir.joinpath('index.json')
...
def _build_tarball(
    ...
    tarfile_dir = cache_prefix.joinpath(tarball_directory_name(spec))
    tarfile_path = tarfile_dir.joinpath(tarfile_name)
    ...
    specfile_path = os.path.realpath(cache_prefix.joinpath(specfile_name))
    ...
    workdir = tmpdir.joinpath(spec.prefix.name)
    # install_tree copies hardlinks
    # create a temporary tarfile from prefix and exract it to workdir
    # tarfile preserves hardlinks
    temp_tarfile_name = tarball_name(spec, '.tar')
    temp_tarfile_path = tarfile_dir.joinpath(temp_tarfile_name)
...
def make_package_relative(workdir, spec, allow_root):
    ...
    for filename in buildinfo['relocate_binaries']:
        orig_path_names.append(prefix.joinpath(filename))
        cur_path_names.append(workdir.joinpath(filename))
    ...
    for linkname in buildinfo.get('relocate_links', []):
        orig_path_names.append(prefix.joinpath(linkname))
        cur_path_names.append(workdir.joinpath(linkname))
...
def relocate_package(spec, allow_root):
    ...
    old_prefix = old_layout_root.joinpath(old_rel_prefix)
    ...
    for filename in buildinfo['relocate_textfiles']:
        text_name = workdir.joinpath(filename)
    ...
    if old_prefix != new_prefix:
        files_to_relocate = [workdir.joinpath(filename)
        ...
        files_to_relocate = list(filter(
            ...
            map(lambda filename: workdir.joinpath(filename),
...
def _extract_inner_tarball(spec, filename, extract_to, unsigned, remote_checksum):
    ...
    spackfile_path = stagepath.joinpath(spackfile_name)
    tarfile_name = tarball_name(spec, '.tar.gz')
    tarfile_path = extract_to.joinpath(tarfile_name)
    deprecated_yaml_name = tarball_name(spec, '.spec.yaml')
    deprecated_yaml_path = extract_to.joinpath(deprecated_yaml_name)
    json_name = tarball_name(spec, '.spec.json')
    json_path = extract_to.joinpath(json_name)
...
def get_keys(install=False, trust=False, force=False, mirrors=None):
    ...
    for mirror in mirror_collection.values():
        ...
        for fingerprint, key_attributes in json_index['keys'].items():
            link = keys_url.joinpath(fingerprint + '.pub')
    ...
    try:
        for mirror in mirrors:
            ...
            for fingerprint in keys:
                ...
                export_target = prefix.joinpath(filename)
...
def needs_rebuild(spec, mirror_url):
    ...
    specfile_name = tarball_name(spec, '.spec.json')
    specfile_path = cache_prefix.joinpath(specfile_name)
...
def _download_buildcache_entry(mirror_root, descriptions):
    for description in descriptions:
        ...
        for url in description['url']:
            description_url = mirror_root.joinpath(url)
            stage = Stage(
...
def download_single_spec(
    ...
    tarball_path_name = tarball_dir_name.joinpath(tarfile_name)
    local_tarball_path = destination.joinpath(tarball_dir_name)
