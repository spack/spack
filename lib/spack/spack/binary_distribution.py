    ...
    def _associate_built_specs_with_mirror(self, cache_key, mirror_url):
        ...
        try:
            ...
            db = spack_db.Database(
                None, db_dir=db_root_dir, enable_transaction_locking=False
            )
                ...
                items_to_remove.append(
                    {
                        ...
                        "cache_key": os.path.join(
                            self._index_cache_root, cached_index_path
                        ),
                    }
                )
...
def generate_package_index(cache_prefix):
    ...
    tty.debug(
        "Retrieving spec descriptor files from {0} to build index".format(cache_prefix)
    )
    ...
    sorted_specs = sorted(
        all_mirror_specs.keys(), key=lambda k: all_mirror_specs[k]["num_deps"]
    )
    ...
    try:
        ...
        for dag_hash in sorted_specs:
            ...
            tty.debug("  {0}/{1} -> {2}".format(s.name, dag_hash[:7], num_deps))
            if num_deps > 0:
                ...
                if to_splice:
                    ...
                    tty.debug("    spliced and wrote {0}".format(spliced_spec_url))
                    spec_record["spec"] = s
...
def generate_key_index(key_prefix, tmpdir=None):
    ...
    index = {
        "keys": dict((fingerprint, {}) for fingerprint in sorted(set(fingerprints)))
    }
...
def _build_tarball(
    ...
    if force:
        ...
        remote_specfile_path_deprecated
    ):
    ...
    with open(spec_file, "r") as inputfile:
        ...
        if spec_file.endswith(".yaml"):
            ...
            raise ValueError(
                "{0} not a valid spec file type (json or yaml)".format(spec_file)
            )
    bchecksum = {}
    ...
    try:
        ...
        if not unsigned:
            push_keys(
                outdir, keys=[key], regenerate_index=regenerate_index, tmpdir=tmpdir
            )
...
def download_tarball(spec, preferred_mirrors=None):
    ...
    if not spack.mirror.MirrorCollection():
        tty.die(
            "Please add a spack mirror to allow " + "download of pre-compiled packages."
        )
    ...
    if preferred_mirrors:
        for preferred_url in preferred_mirrors:
            urls_to_try.append(
                url_util.join(preferred_url, _build_cache_relative_path, tarball)
            )

    for mirror in spack.mirror.MirrorCollection().values():
        if not preferred_mirrors or mirror.fetch_url not in preferred_mirrors:
            urls_to_try.append(
                url_util.join(mirror.fetch_url, _build_cache_relative_path, tarball)
            )
...
def relocate_package(spec, allow_root):
    ...
    tty.debug(
        "Relocating package from", "%s to %s." % (old_layout_root, new_layout_root)
    )
...
def extract_tarball(spec, filename, allow_root=False, unsigned=False, force=False):
    ...
    extracted_dir = os.path.join(
        extract_tmp, old_relative_prefix.split(os.path.sep)[-1]
    )
            ...
            try:
                ...
                tty.debug(
                    "Did not find {0} on {1}".format(
                        specfile_name, buildcache_fetch_url_json
                    ),
                    url_err,
                )
                tty.debug(
                    "Did not find {0} on {1}".format(
                        specfile_name, buildcache_fetch_url_yaml
                    ),
                    url_err_y,
                )
                continue
...
def get_mirrors_for_spec(
    spec=None, full_hash_match=False, mirrors_to_check=None, index_only=False
):
    """
    ...
    if not results and not index_only:
        results = try_direct_fetch(
            spec, full_hash_match=full_hash_match, mirrors=mirrors_to_check
        )
...
def needs_rebuild(spec, mirror_url, rebuild_on_errors=False):
    ...
    tty.debug(
        "Checking {0}-{1}, dag_hash = {2}, full_hash = {3}".format(
    ...
    deprecated_specfile_path = os.path.join(cache_prefix, deprecated_specfile_name)
    ...
    try:
        ...
        try:
            ...
            tty.error(
                "".join(err_msg).format(
                    spec.short_spec, specfile_path, deprecated_specfile_path
                )
            )
            tty.debug(url_err)
            tty.debug(url_err_y)
    ...
    if not spec_file_contents:
        tty.error(
            "Reading {0} returned nothing".format(
    ...
    spec_dict = (
        sjson.load(spec_file_contents)
        if specfile_is_json
        else syaml.load(spec_file_contents)
    )
    ...
    if nodes and "name" not in nodes[0]:
        ...
        cached_pkg_specs = [
            item for item in nodes if item[ht.dag_hash.name] == spec.dag_hash()
        ]
    ...
    if rebuild:
        tty.msg("Rebuilding {0}, reason: {1}".format(spec.short_spec, reason))
        tty.msg(spec.tree())

    return rebuild


def check_specs_against_mirrors(
    mirrors, specs, output_file=None, rebuild_on_errors=False
):
    ...
    for mirror in spack.mirror.MirrorCollection(mirrors).values():
        ...
        for spec in specs:
            if needs_rebuild(spec, mirror.fetch_url, rebuild_on_errors):
                rebuild_list.append(
                    {"short_spec": spec.short_spec, "hash": spec.dag_hash()}
                )
...
def download_buildcache_entry(file_descriptions, mirror_url=None):
    if not mirror_url and not spack.mirror.MirrorCollection():
        tty.die(
            "Please provide or add a spack mirror to allow "
