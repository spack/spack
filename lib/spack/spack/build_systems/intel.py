...
class IntelPackage(PackageBase):
    ...
    def normalize_suite_dir(self, suite_dir_name, version_globs=["*.*.*"]):
        ...
        if suite_dir_name and suite_dir_name in d:
            ...
            d = re.sub(
                "(%s%s.*?)%s.*" % (os.sep, re.escape(suite_dir_name), os.sep), r"\1", d
            )
            ...
            if self.version_yearlike >= ver("2019"):
                ...
                if "+external-libfabric" in self.spec:
                    ...
                    result += find_libraries(
                        ["libfabric"], os.path.join(d, "libfabric", "lib")
                    )

        if "^mpi" in self.spec.root and (
            "+mkl" in self.spec or self.provides("scalapack")
        ):
    ...
    def _setup_dependent_env_callback(
        ...
        if "+mkl" in self.spec or self.provides("mkl"):
            ...
            env.append_path(
                "SPACK_COMPILER_EXTRA_RPATHS", env_mods["SPACK_COMPILER_EXTRA_RPATHS"]
            )
    ...
    def setup_dependent_package(self, module, dep_spec):
        ...
        if "+mpi" in self.spec or self.provides("mpi"):
            ...
            debug_print(
                ("spec '%s' received .mpi* properties:" % self.spec),
