...
def setup_parser(subparser):
    ...
    keys.add_argument(
        "-t", "--trust", action="store_true", help="trust all downloaded keys"
    )
    keys.add_argument(
        "-f", "--force", action="store_true", help="force new download of keys"
    )
    ...
    check.add_argument(
        ...
        default=None,
        help=(
            "Check single spec from json or yaml file instead of release "
            + "specs file"
        ),
    )
    ...
    savespecfile.add_argument(
        "--root-spec", default=None, help="Root spec of dependent spec"
    )
    savespecfile.add_argument(
        "--root-specfile",
        default=None,
...
def check_fn(args):
    ...
    sys.exit(
        bindist.check_specs_against_mirrors(
...
def copy_fn(args):
    ...
    if dest_url[0:7] != "file://" and dest_url[0] != "/":
        tty.msg(
            'Only urls beginning with "file://" or "/" are supported '
            + "by buildcache copy."
        )
        sys.exit(1)
...
def sync_fn(args):
    ...
    if args.src_directory:
        ...
        if result.name == "<unnamed>":
            raise ValueError(
                'no configured mirror named "{name}"'.format(name=source_location)
            )
    ...
    if args.dest_directory:
        ...
        if result.name == "<unnamed>":
            raise ValueError(
                'no configured mirror named "{name}"'.format(name=dest_location)
            )
    ...
    try:
        for rel_path in buildcache_rel_paths:
            ...
            tty.debug(
                "Copying {0} to {1} via {2}".format(src_url, dest_url, local_path)
            )
