...
def ci_rebuild(args):
    ...
    if "enable-artifacts-buildcache" in gitlab_ci:
        ...
        if enable_artifacts_mirror or (
            ...
            and not enable_artifacts_mirror
    ...
    if pipeline_mirror_url:
        spack.mirror.add(
            spack_ci.TEMP_STORAGE_MIRROR_NAME,
            pipeline_mirror_url,
    ...
    if matches:
        ...
        tty.msg(
            "No need to rebuild {0}, found full hash match at: ".format(
                job_spec_pkg_name
            )
        )
        ...
        if enable_artifacts_mirror:
            ...
            tty.debug(
                "Getting {0} buildcache from {1}".format(
    ...
    if install_exit_code == 0:
        ...
        if buildcache_mirror_url:
            ...
            if cdash_build_id:
                tty.debug(
                    "Writing cdashid ({0}) to remote mirror: {1}".format(
        ...
        if spack_is_develop_pipeline and "broken-specs-url" in gitlab_ci:
            ...
            if web_util.url_exists(broken_spec_path):
                tty.msg(
                    "Removing {0} from the list of broken specs".format(
                        broken_spec_path
                    )
                )
                try:
