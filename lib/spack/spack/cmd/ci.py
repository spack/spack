...
def setup_parser(subparser: argparse.ArgumentParser) -> None:
    ...
    spack.cmd.common.arguments.add_common_arguments(test, ["jobs"])
...
def ci_rebuild(args):
    ...
    if install_exit_code == FAIL_CODE and spack_is_develop_pipeline:
...
def ci_test(args):
    ...
    check a single spec against the local mirror, build it if the mirror
    contains the hash, and run its stand-alone tests
    """
    ...
    # Is this a pipeline run on a spack PR or a merge to develop?  It might
    # be neither, e.g. a pipeline run on some environment repository.
    # TODO/TBD: Does this matter for stand-alone testing?
    spack_is_pr_pipeline = spack_pipeline_type == "spack_pull_request"
    spack_is_develop_pipeline = spack_pipeline_type == "spack_protected_branch"
    ...
    # TODO/TBD/TLD: Don't clear the cdash report or (build) job log directory
    ...
    # Skip stand-alone testing if the package is known to be "broken" for CI.
    broken_tests = (
        "broken-tests-packages" in ci_config
        and job_spec.name in ci_config["broken-tests-packages"]
    )
    ...
    for match in matches:
        tty.msg(f"    {match.url_and_version.url}")
    ...
    # Now do the post-install tasks
    tty.debug(f"spack cache-only install exited with {install_exit_code}")

    # Copy logs and archived files from the install metadata (.spack) directory to artifacts now
    spack_ci.copy_stage_logs_to_artifacts(job_spec, job_test_dir)
    ...
    if install_exit_code != 0:
        if cdash_handler:
            msg = "Failed to install the package from cache"
            cdash_handler.report_skipped(job_spec, reports_dir, reason=msg)
            cdash_handler.copy_test_results(reports_dir, job_test_dir)
        tty.die("Unable to run stand-alone tests due to unsuccessful installation")

    test_exit_code = FAIL_CODE
    ...
    try:
        ...
        # If there is any error, just print a warning.
        tty.warn(f"Error processing stand-alone tests: {str(err)}")
        test_exit_code = FAIL_CODE
