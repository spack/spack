...
# TODO/TLD: Update this to install from cache and run unit tests
def ci_test(args):
    """test a spec if it on the remote mirror

    check a single spec against the remote mirror, build it if the mirror
    ...
    spack_pipeline_type = os.environ.get("SPACK_PIPELINE_TYPE")
    spack_ci_stack_name = os.environ.get("SPACK_CI_STACK_NAME")
    #TLD rebuild_everything = os.environ.get("SPACK_REBUILD_EVERYTHING")
    require_signing = os.environ.get("SPACK_REQUIRE_SIGNING")
    ...
    #TLD full_rebuild = True if rebuild_everything and rebuild_everything.lower() == "true" else False

    pipeline_mirrors = spack.mirrors.mirror.MirrorCollection(binary=True)
    buildcache_destination = None
    if "buildcache-destination" not in pipeline_mirrors:
        tty.die("spack ci test requires a mirror named 'buildcache-destination")
    ...
    # Start with spack arguments
    # TLD/TODO: how does this change? .. need to install from cache
    spack_cmd = [SPACK_COMMAND, "--color=always", "--backtrace", "--verbose", "install"]
