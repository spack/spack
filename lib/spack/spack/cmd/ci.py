...
def ci_generate(args):
    ...
    env = spack.cmd.get_env_for_command('ci generate', required=True)
...
def ci_reindex(args):
    """Rebuild the buildcache index associated with the mirror in the
       active, gitlab-enabled environment. """
    env = spack.cmd.get_env_for_command('ci rebuild-index', required=True)
...
def ci_rebuild(args):
    """Check a single spec against the remote mirror, and rebuild it from
       ...
       as computed locally. """
    env = spack.cmd.get_env_for_command('ci rebuild', required=True)
