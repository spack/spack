...
def setup_parser(subparser):
    ...
    add_parser.add_argument(
        "-f", "--file", help="file from which to set all config values"
    )
    ...
    remove_parser = sp.add_parser(
        "remove", aliases=["rm"], help="remove configuration parameters"
    )
...
def config_get(args):
    ...
    if section is not None:
        ...
        tty.die(
            "`spack config get` requires a section argument "
...
def config_edit(args):
    ...
    if spack_env and not args.scope:
        ...
        if not scope and not section:
            tty.die(
                "`spack config edit` requires a section argument "
                "or an active environment."
            )
        config_file = spack.config.config.get_config_filename(scope, section)
...
def config_revert(args):
    scopes = (
        [args.scope]
        if args.scope
        else [x.name for x in spack.config.config.file_scopes]
    )
    ...
    if not args.yes_to_all:
        msg = (
            "The following scopes will be restored from the corresponding"
            " backup files:\n"
        )
