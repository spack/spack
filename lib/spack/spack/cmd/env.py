...
def env_activate(args):
    ...
    env_mods.extend(
        spack.environment.shell.activate(env=active_env, add_view=args.with_view)
    )
...
def _env_create(
    name_or_path, init_file=None, dir=False, with_view=None, keep_relative=False
):
...
def env_remove_setup_parser(subparser):
    ...
    subparser.add_argument(
        "rm_env", metavar="env", nargs="+", help="environment(s) to remove"
    )
