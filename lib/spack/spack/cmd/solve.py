...
class SolveUI(HeadlessUI):
    ...
    def on_solve_finished(
        ...
        *,
        ...
        # A solve that was only set up has no result, no statistics and no complete timings
        if result is None:
            return
    ...
    def _print_solutions(self, result: Result) -> None:
        """Dump the solutions of a solve as concretized specs."""
        if self.required_format:
            for spec in result.specs:
                # With -y, just print YAML to output.
                if self.required_format == "yaml":
                    # use write because to_yaml already has a newline.
                    sys.stdout.write(spec.to_yaml())
                elif self.required_format == "json":
                    print(spec.to_json())
                else:
                    print(spec.format(self.required_format))
        else:
            tree_str = spack.spec.tree(
                result.specs, color=color.get_color_when(sys.stdout), **self.tree_kwargs
            )
...
def _solve(
    solver: asp.Solver, specs: Sequence[Spec], *, kind: SolveKind, setup_only: bool
) -> None:
    """Run each of the solves that concretize ``specs``. Their output is rendered by the
    frontend of ``solver``.
    """
    if kind is SolveKind.WHEN_POSSIBLE:
        # solve_in_rounds has no setup_only: each round depends on the result of the previous one
        for _ in solver.solve_in_rounds(specs):
            pass
        return

    inputs = [[s] for s in specs] if kind is SolveKind.SEPARATELY else [list(specs)]
    for current in inputs:
        solver.solve(current, setup_only=setup_only)
...
def solve(parser, args):
    ...
    # The frontend renders each solve, and no concretized spec is reported, hence total=0
    with concretization_span(ui), group_span(
        ui, group=DEFAULT_USER_SPEC_GROUP, kind=kind, total=0, processes=1
    ):
        _solve(
            asp.Solver(context=spack.context.default(), ui=ui),
            specs,
            kind=kind,
