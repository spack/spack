...
def _has_any_debug_binary(prefix: str) -> bool:
    """Fast pre-check: does this prefix contain at least one ELF binary
    with debug sections? Used to skip the full debug-capture pipeline
    for packages built without any debug info (-g, build_type=Debug/
    RelWithDebInfo, or otherwise), regardless of how that was set."""
    for root, _, files in os.walk(prefix):
        for filename in files:
            filepath = os.path.join(root, filename)
            if os.path.islink(filepath):
                continue
            if _is_elf_with_debug_sections(filepath):
                return True
    return False
