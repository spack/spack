...
class DirectoryLayout(object):
    ...
    * <install root>/
        ...
        try:
            with open(path) as f:
                ...
                if extension == ".json":
                    ...
                    raise SpecReadError(
                        "Did not recognize spec file extension:"
                        " {0}".format(extension)
                    )
    ...
    def check_installed(self, spec):
        ...
        elif installed_spec.copy(deps=("link", "run")) == spec.copy(
            deps=("link", "run")
        ):
    ...
    def all_deprecated_specs(self):
        ...
        for _, path_scheme in self.projections.items():
            ...
            deprecated_specs |= set(
                (self.read_spec(s), self.read_spec(get_depr_spec_file(s)))
                for s in spec_files
            )
...
class SpecHashCollisionError(DirectoryLayoutError):
    ...
    def __init__(self, installed_spec, new_spec):
        super(SpecHashCollisionError, self).__init__(
            "Specs %s and %s have the same SHA-1 prefix!" % (installed_spec, new_spec)
        )
