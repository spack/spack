...
class Environment(object):
    def __init__(self, path, init_file=None, with_view=None, keep_relative=False):
        ...
        if with_view is False:
            ...
            self.views = {
                default_view_name: ViewDescriptor(self.path, self.view_path_default)
            }
    ...
    def _rewrite_relative_paths_on_relocation(self, init_file_dir):
        ...
        for name, entry in self.dev_specs.items():
            ...
            tty.debug(
                "Expanding develop path for {0} to {1}".format(name, expanded_path)
            )
    ...
    def update_stale_references(self, from_list=None):
        ...
        for i, (name, speclist) in enumerate(
            ...
            new_reference = dict(
                (n, self.spec_lists[n]) for n in list(self.spec_lists.keys())[:i]
            )
    ...
    def remove(self, query_spec, list_name=user_speclist_name, force=False):
        ...
        if not matches:
            ...
            specs_hashes = zip(self.concretized_user_specs, dag_hashes_in_order)
            ...
            for view_name, view in self.views.items():
                for _, spec in self.concretized_specs():
                    if spec in view and spec.package.installed:
                        tty.debug("Spec %s in view %s" % (spec.name, view_name))
    ...
    def rm_default_view_from_env(self, env_mod):
        ...
        env_mod.extend(
            uenv.unconditional_environment_modifications(self.default_view).reversed()
        )
    ...
    def _get_overwrite_specs(self):
        ...
        changed_dev_specs = set(
            s for s in specs_to_check if _is_dev_spec_and_has_changed(s)
        )

        return [
            s.dag_hash()
            for s in specs_to_check
            if _spec_needs_overwrite(s, changed_dev_specs)
        ]
    ...
    def install_specs(self, specs=None, **install_args):
        ...
        install_args["overwrite"] = (
            install_args.get("overwrite", []) + specs_to_overwrite
        )
            ...
            for spec in specs_to_install:
                if spec.package.installed:
                    ...
                    try:
                        ...
                        tty.warn(
                            "Could not install log links for {0}: {1}".format(
                                spec.name, str(e)
                            )
                        )
    ...
    def matching_spec(self, spec):
        ...
        for user_spec, concretized_user_spec in self.concretized_specs():
            ...
            if spec.concrete:
                ...
                is_match = (
                    spec in concretized_user_spec
                    and concretized_user_spec[spec.name].build_hash()
                    == spec.build_hash()
                )
                if is_match:
                    matches[spec] = spec
    ...
    def _get_environment_specs(self, recurse_dependencies=True):
        ...
        for spec_hash in self.concretized_order:
            ...
            specs = (
                spec.traverse(deptype=("link", "run"))
    ...
    def write(self, regenerate=True):
        ...
        if manifest_exists and not is_latest_format(self.manifest_path):
            msg = (
                'The environment "{0}" needs to be written to disk, but '
...
def _equiv_dict(first, second):
    ...
    if len(first) != len(second):
        return False
    same_values = all(
        yaml_equivalent(fv, sv) for fv, sv in zip(first.values(), second.values())
    )
    ...
    while True:
        ...
        try:
            ...
            inv_variant_constraints = [
                c
                for c in spec_constraints
                if any(name in c.variants for name in invalid_variants)
            ]
