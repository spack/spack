...
def get_checksums(spec):
    ...
    commit = getattr(spec.variants.get("commit"), "value", None)
    if commit and spec.package.needs_commit(spec.version):
        checksums.append({"algorithm": "SHA1", "checksumValue": commit})
