...
def _win_rename(src, dst):
    ...
    # most cases but diverges from POSIX rename in two ways:
    #   1. dst is a symlink to a directory MoveFileExW fails; remove the
    #      symlink first.
    #   2. dst is a plain directory MoveFileExW returns ERROR_ACCESS_DENIED
    ...
    if os.path.islink(dst) and os.path.isdir(os.path.realpath(dst)):
        ...
        os.rmdir(dst)  # only passes if empty, same as POSIX rename
    os.replace(src, dst)
