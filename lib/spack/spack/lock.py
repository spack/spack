...
"""Wrapper for ``spack.util.lock`` allows locking to be enabled/disabled."""
import os
import stat
import sys
...
import spack.error
from spack.util.lock import Lock as Llnl_lock
...
class Lock(Llnl_lock):
    ...
    ``spack.util.lock`` so that all the lock API calls will succeed, but
