...
def get_module(
    module_type, spec, get_full_path, module_set_name="default", required=True
):
    ...
    if upstream:
        ...
        if not os.path.isfile(writer.layout.filename):
            if not writer.conf.blacklisted:
                ...
                tty.debug(
                    "The module configuration has blacklisted {0}: "
...
class BaseContext(tengine.Context):
    ...
    def environment_modifications(self):
        ...
        env.extend(
            spack.build_environment.modifications_from_dependencies(spec, context="run")
        )
    ...
    def update_module_defaults(self):
        if any(self.spec.satisfies(default) for default in self.conf.defaults):
            ...
            default_path = os.path.join(
                os.path.dirname(self.layout.filename), "default"
            )
