...
class PackageBase(six.with_metaclass(PackageMeta, PackageViewMixin, object)):
    ...
    **Package Lifecycle**
    ...
    def uninstall_by_spec(spec, force=False, deprecator=None):
        ...
        if not force:
            dependents = store.db.installed_relatives(spec, 'parents', True)
            if dependents:
        ...
        with store.db.prefix_write_lock(spec):
    ...
    def do_deprecate(self, deprecator, link_fn):
        ...
        if old_deprecator:
            ...
            self_yaml = store.layout.deprecated_file_path(spec, old_deprecator)
        else:
            self_yaml = store.layout.spec_file_path(spec)

        # copy spec metadata to "deprecated" dir of deprecator
        depr_yaml = store.layout.deprecated_file_path(spec, deprecator)
    ...
    def do_activate(self, view=None, with_dependencies=True, verbose=True):
        ...
        if not view:
            view = YamlFilesystemView(
                self.extendee_spec.prefix, spack.store.store.layout)
