...
class CDash(Reporter):
    ...
    def __init__(self, args):
        ...
        if args.cdash_buildstamp:
            ...
            self.buildstamp = time.strftime(
                buildstamp_format, time.localtime(self.endtime)
            )
        ...
        for line in package["stdout"].splitlines():
            ...
            if match:
                ...
                report_data[cdash_phase]["loglines"].append(
                    text_type(
                        "{0} output for {1}:".format(cdash_phase, package["name"])
                    )
                )
            elif cdash_phase:
                report_data[cdash_phase]["loglines"].append(
    ...
    def build_report(self, directory_name, input_data):
        ...
        for spec in input_data["specs"]:
            ...
            spec["packages"] = [
                ...
                if "installed_from_binary_cache" not in x
                or not x["installed_from_binary_cache"]
            ]
