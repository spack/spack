...
def deprecate_paths_and_modules(instance, deprecated_properties):
    ...
    keys_to_be_removed = [x for x in instance_copy if x not in deprecated_properties]
    ...
    msg = (
        "detected deprecated properties in {0}\nActivate the debug "
        "flag to have more information on the deprecated parts or "
        "run:\n\n\t{2}\n\nto update the file to the new format\n"
    )
    return msg.format(absolute_path, yaml_as_str, command_to_suggest)
...
def update(data):
    ...
    for cfg_object in data.values():
        ...
        for spec, prefix in paths.items():
            externals.append({"spec": str(spec), "prefix": str(prefix)})
        modules = cfg_object.pop("modules", {})
        for spec, module in modules.items():
            externals.append({"spec": str(spec), "modules": [str(module)]})
        if externals:
