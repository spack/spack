    ...
    def format(self, format_string='$_$@$%@+$+$=', **kwargs):
        ...
        for i, c in iterator:
            if escape:
                ...
                transform = token_transforms.get(attr_id, lambda x: x)
                if attr_id in ['PACKAGE', 'NAME']:
                    name = spec.name or ''
                    write(fmt % transform(spec.name), '@')
                    ...
                    if versionStr:
                        write(fmt % transform(versionStr), '@')
                elif attr_id == 'COMPILER':
                    if spec.compiler:
                        write(fmt % transform(spec.compiler), '%')
                elif attr_id == 'COMPILERNAME':
                    if spec.compiler:
                        write(fmt % transform(spec.compiler.name), '%')
                elif attr_id in ['COMPILERVER', 'COMPILERVERSION']:
                    if spec.compiler:
                        write(
                            fmt % transform(spec.compiler.versions), '%')
                elif attr_id == 'COMPILERFLAGS':
                    if spec.compiler:
                        write(fmt % transform(str(spec.compiler_flags)), '%')
                elif attr_id == 'OPTIONS':
                    if spec.variants:
                        write(fmt % transform(str(spec.variants)), '+')
                elif attr_id == 'ARCHITECTURE':
                    if spec.architecture and str(spec.architecture):
                        write(fmt % transform(str(self.architecture)), '=')
                elif attr_id == 'SHA1':
                    if spec.dependencies:
                        out.write(fmt % transform(str(spec.dag_hash(7))))
                elif attr_id.startswith('VARIANT'):
                    _, setting, ifTrueStr = attribute.split(':')
                    if setting in spec:
                        out.write(fmt % transform(ifTrueStr))
                elif attr_id == 'SPACK_ROOT':
                    out.write(fmt % transform(spack.prefix))
                elif attr_id == 'SPACK_INSTALL':
                    out.write(fmt % transform(spack.store.root))
                elif attr_id == 'PREFIX':
                    out.write(fmt % transform(spec.prefix))
