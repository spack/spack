...
class Spec:
    ...
    def _finalize_concretization(self):
        ...
        # Assign dag_hash (this *could* be done lazily, but the post-concretization deprecation
        # check looks specs up by dag_hash, so it's assigned anyway; it's clearer to see
        # explicitly where it happens). Any specs that were concrete before finalization will
        # already have a cached DAG hash.
        for spec in self.traverse():
    ...
    def attach_git_version_lookup(self, *, repo=None, misc_cache=None, config=None):
        ...
        for v in self.versions:
            if isinstance(v, vn.GitVersion) and v.std_version is None:
                v.attach_lookup(
                    spack.version.git_ref_lookup.GitRefLookup(
                        self.fullname, repo=repo, misc_cache=misc_cache, config=config
                    )
                )
