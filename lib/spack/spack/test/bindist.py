...
def test_install_legacy_yaml(
    test_legacy_mirror, install_mockery_mutable_config, mock_packages
):
    install_cmd(
        "--no-check-signature",
        "--cache-only",
...
def test_update_index_fix_deps(monkeypatch, tmpdir, mutable_config):
    ...
    b_spec_json_path = os.path.join(
        mirror_dir.strpath, bindist.build_cache_relative_path(), b_spec_json_name
    )
    fs.filter_file(
        r'"full_hash":\s"\S+"',
    ...
    a_spec_json_path = os.path.join(
        mirror_dir.strpath, bindist.build_cache_relative_path(), a_spec_json_name
    )
