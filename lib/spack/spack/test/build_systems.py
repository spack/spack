...
@pytest.mark.parametrize(
    "directory", glob.iglob(os.path.join(DATA_PATH, "make", "affirmative", "*"))
)
...
@pytest.mark.parametrize(
    "directory", glob.iglob(os.path.join(DATA_PATH, "make", "negative", "*"))
)
...
@pytest.mark.parametrize(
    "directory", glob.iglob(os.path.join(DATA_PATH, "ninja", "affirmative", "*"))
)
def test_affirmative_ninja_check(directory, config, mock_packages, working_env):
...
""".format(
        ...
        msg = (
            "Spack could not find `config.guess`.*misconfigured as an "
            "external package"
        )
...
class TestGNUMirrorPackage(object):
    def test_define(self):
        ...
        with pytest.raises(
            ...
            r"attribute \[none defined\]".format(cls_name),
        ):
            pkg_broken.urls

        assert pkg.urls[0] == "https://ftpmirror.gnu.org/" "make/make-4.2.1.tar.gz"
...
class TestSourceforgePackage(object):
    def test_define(self):
        ...
        assert (
            pkg.urls[0] == "https://prdownloads.sourceforge.net/"
...
class TestSourcewarePackage(object):
    def test_define(self):
        ...
        assert pkg.urls[0] == "https://sourceware.org/pub/" "bzip2/bzip2-1.0.8.tar.gz"
...
class TestXorgPackage(object):
    def test_define(self):
        ...
        assert (
            pkg.urls[0] == "https://www.x.org/archive/individual/"
