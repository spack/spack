...
def test_ccld_mode(wrapper_environment):
    ...
    assert (
        dump_mode(cc, ["foo.o", "bar.o", "baz.o", "-o", "foo", "-Wl,-rpath,foo"])
        == "ccld"
    )
...
def test_ld_mode(wrapper_environment):
    ...
    assert (
        dump_mode(ld, ["foo.o", "bar.o", "baz.o", "-o", "foo", "-Wl,-rpath,foo"])
        == "ld"
    )
...
def test_ld_deps_no_link(wrapper_environment):
    ...
    with set_env(
        SPACK_INCLUDE_DIRS="xinc:yinc:zinc", SPACK_RPATH_DIRS="xlib:ylib:zlib"
    ):
        check_args(
            ld,
            test_args,
...
def test_ld_deps_partial(wrapper_environment):
    ...
    """
    with set_env(
        SPACK_INCLUDE_DIRS="xinc", SPACK_RPATH_DIRS="xlib", SPACK_LINK_DIRS="xlib"
    ):
