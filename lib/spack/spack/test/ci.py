...
def test_configure_compilers(mutable_config):
    ...
    def assert_present(config):
        assert (
            "install_missing_compilers" in config
...
def test_register_cdash_build(monkeypatch):
    ...
    response_obj = {"buildid": 42}

    fake_responder = FakeWebResponder(content_to_read=[json.dumps(response_obj)])
...
def test_relate_cdash_builds(
    config, mutable_mock_env_path, mock_packages, monkeypatch, capfd
):
    e = ev.create("test1")
    e.add("dyninst")
    e.concretize()
    ...
    with e as active_env:
        ...
        dep_cdash_ids = {"libdwarf": 1, "libelf": 2}

        monkeypatch.setattr(
            ci, "read_cdashid_from_mirror", lambda s, u: dep_cdash_ids.pop(s.name)
        )

        fake_responder = FakeWebResponder(content_to_read=["libdwarf", "libelf"])
        monkeypatch.setattr(ci, "build_opener", lambda handler: fake_responder)

        ci.relate_cdash_builds(
            spec_map, cdash_api_url, job_build_id, cdash_project, [cdashids_mirror_url]
        )
        ...
        ci.relate_cdash_builds(
            spec_map, cdash_api_url, None, cdash_project, [cdashids_mirror_url]
        )
...
def test_ci_workarounds():
    ...
    common_before_script = [
        ...
        " && ".join(
            ("pushd ./spack", 'git checkout "{ref}"'.format(ref=fake_spack_ref), "popd")
        ),
        '. "./spack/share/spack/setup-env.sh"',
    ]

    def make_build_job(
        name, deps, stage, use_artifact_buildcache, optimize, use_dependencies
    ):
        ...
        if use_dependencies:
            ...
            result["needs"] = [
                {"job": dep, "artifacts": use_artifact_buildcache} for dep in deps
            ]
    ...
    def make_yaml_obj(use_artifact_buildcache, optimize, use_dependencies):
        ...
        result.update(
            make_rebuild_index_job(use_artifact_buildcache, optimize, use_dependencies)
        )
