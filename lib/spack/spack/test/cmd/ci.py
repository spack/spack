...
def test_ci_generate_protected_pipeline(tmpdir, mutable_mock_env_path,
                                        install_mockery,
                                        mock_packages, monkeypatch,
                                        ci_base_environment):
    ...
    os.environ.update({
        ...
        'SPACK_SIGNING_MODE': 'External',
        'CI_JOB_NAME': 'genpipeline',
        'CI_PIPELINE_ID': '42'
    })
    filename = str(tmpdir.join('spack.yaml'))
    with open(filename, 'w') as f:
...
""")
    ...
    with tmpdir.as_cwd():
        ...
        with ev.read('test'):
            gen_out = ci_cmd('generate', '--output-file', outputfile, output=str)
            print(gen_out)
        ...
        with open(outputfile) as f:
            ...
            sign_pkgs_job = yaml_contents['sign-pkgs']
            assert('needs' in sign_pkgs_job)
            signing_needs = sign_pkgs_job['needs']
            assert(len(signing_needs) == 2)
            needs_names = [job_needs['job'] for job_needs in signing_needs]
            assert('genpipeline' in needs_names)
            final_job = [job for job in needs_names if 'flatten-deps' in job]
            assert(len(final_job) == 1)
