    ...
    for job_name, job_def in yaml_contents.items():
        for needs_def_name, needs_list in needs_graph.items():
            if job_name.startswith(needs_def_name):
                ...
                assert all(
                    [
                        nl in [n["job"][: n["job"].index("/")] for n in j_needs]
                        for nl in needs_list
                    ]
                )
                assert all(
                    [job_needs["artifacts"] == artifacts for job_needs in j_needs]
                )
                break


def test_ci_generate_bootstrap_gcc(
    tmpdir, mutable_mock_env_path, install_mockery, mock_packages, project_dir_env
):
...
"""
    ...
    with tmpdir.as_cwd():
        ...
        with ev.read("test"):
            ...
            monkeypatch.setattr(ci, "SPACK_PR_MIRRORS_ROOT_URL", r"file:///fake/mirror")
            monkeypatch.setattr(
...
""".format(
    ...
    with tmpdir.as_cwd():
        ...
        with ev.read("test") as env:
            ...
            shutil.copyfile(
                env.manifest_path, os.path.join(env_dir.strpath, "spack.yaml")
            )
...
""".format(
    ...
    buildcache_cmd(
        "create", "-a", "-f", "-u", "--mirror-url", mirror_url, "archive-files"
    )
...
""".format(
    ...
    with tmpdir.as_cwd():
        ...
        with ev.read("test") as env:
            ...
            buildcache_cmd(
                "create", "-u", "-a", "-f", "--mirror-url", mirror_url, "callpath"
            )
...
def test_ci_generate_bootstrap_prune_dag(
    ...
    buildcache_cmd(
        "create", "-u", "-a", "-f", "-d", mirror_dir.strpath, "gcc@10.1.0%gcc@4.5.0"
    )
    ...
    monkeypatch.setattr(
        spack.concretize.Concretizer, "check_for_compiler_existence", False
    )
...
""".format(
    ...
    with tmpdir.as_cwd():
        ...
        with ev.read("test") as env:
            ...
            shutil.copyfile(
                env.manifest_path, os.path.join(working_dir.strpath, "spack.yaml")
            )
            shutil.copyfile(
                env.lock_path, os.path.join(working_dir.strpath, "spack.lock")
            )
            ...
            job_spec_yaml_path = os.path.join(working_dir.strpath, "archivefiles.yaml")
            with open(job_spec_yaml_path, "w") as fd:
                fd.write(job_spec.to_yaml(hash=ht.full_hash))

            root_spec_yaml_path = os.path.join(working_dir.strpath, "root.yaml")
    ...
    monkeypatch.setattr(
        ci, "download_and_extract_artifacts", fake_download_and_extract_artifacts
    )
    rep_out = ci_cmd(
        "reproduce-build",
