...
def test_config_add_to_env_preserve_comments(
    mutable_empty_config, mutable_mock_env_path, tmpdir
):
...
def test_config_update_fail_on_permission_issue(packages_yaml_v015, monkeypatch):
    # The first time it will update and create the backup file
    packages_yaml_v015()
    # Mock a global scope where we cannot write
    monkeypatch.setattr(spack.cmd.config, "_can_update_config_file", lambda x, y: False)
...
def test_config_revert_raise_if_cant_write(packages_yaml_v015, monkeypatch):
    ...
    # Mock a global scope where we cannot write
    monkeypatch.setattr(spack.cmd.config, "_can_revert_update", lambda x, y, z: False)
