...
@pytest.mark.parametrize('args,name,expected', [
    ...
    (['-n', 'example-auto-git', 'file:///tmp/example.git'], 'example-auto-git',
     [r'ExampleAutoGit(Package)', r'def install(self',
      r'git      = "file:///tmp/example.git']),
    (['-n', 'example-auto-git-2', 'git@example.com:tmp/example.git'],
     'example-auto-git-2',
     [r'ExampleAutoGit2(Package)', r'def install(self',
      r'git      = "git@example.com:tmp/example.git']),
    (['-n', 'example-force-git', '-g', 'file://example.tar.gz'], 'example-force-git',
     [r'ExampleForceGit(Package)', r'def install(self',
      r'git      = "file://example.tar.gz']),
    # Test git branch/tag/commit
    (['-n', 'example-git-branch', '-g', 'file:///tmp/example.git', '-B', 'v3.2.1'],
     'example-git-branch',
     [r'ExampleGitBranch(Package)', r'def install(self',
      r'git      = "file:///tmp/example.git',
      r"version('v3.2.1', branch='v3.2.1')"]),
    (['-n', 'example-git-branch2', '-g', 'file:///tmp/example.git', '-B', 'v3.2.1',
      '-V', 'v1.2.3'],
     'example-git-branch2',
     [r'ExampleGitBranch2(Package)', r'def install(self',
      r'git      = "file:///tmp/example.git',
      r"version('v1.2.3', branch='v3.2.1')"]),
    (['-n', 'example-git-tag', '-g', 'file:///tmp/example.git', '-T', 'v4.3.2',
      '-V', 'v2.3.4'],
     'example-git-tag',
     [r'ExampleGitTag(Package)', r'def install(self',
      r'git      = "file:///tmp/example.git',
      r"version('v2.3.4', tag='v4.3.2')"]),
    (['-n', 'example-git-commit', '-g', 'file:///tmp/example.git', '-C',
      'babc42a34cb565760ca875483de75ff94b2bc112'],
     'example-git-commit',
     [r'ExampleGitCommit(Package)', r'def install(self',
      r'git      = "file:///tmp/example.git',
