...
def create_v1_lockfile_dict(roots, all_specs):
    test_lockfile_dict = {
        "_meta": {"lockfile-version": 1, "file-type": "spack-lockfile"},
        "roots": list({"hash": s.dag_hash(), "spec": s.name} for s in roots),
        ...
        "concrete_specs": dict(
            (s.dag_hash(), s.to_node_dict(hash=ht.build_hash)) for s in all_specs
        ),
    }
...
def test_env_view_fails(tmpdir, mock_packages, mock_stage, mock_fetch, install_mockery):
    ...
    with ev.read("test"):
        ...
        add("libelf cflags=-g")
        with pytest.raises(
            llnl.util.link_tree.MergeConflictError, match="merge blocked by file"
        ):
            install("--fake")
    ...
    with tmpdir.as_cwd():
        ...
        for user, concrete in test.concretized_specs():
            ...
            if user.name == "mpileaks":
                assert (
                    concrete.variants["shared"].value == user.variants["shared"].value
                )
...
"""
    ...
    with tmpdir.as_cwd():
        ...
        packages_lists = list(
            filter(lambda x: "packages" in x, test.yaml["env"]["definitions"])
        )
    ...
    with tmpdir.as_cwd():
        ...
        for spec in test._get_environment_specs():
            assert os.path.exists(
                os.path.join(
                    viewdir, spec.name, "%s-%s" % (spec.version, spec.compiler.name)
                )
            )
            ...
            if not spec.satisfies("callpath%gcc"):
                assert os.path.exists(
                    os.path.join(
                        combin_viewdir,
                        spec.name,
                        "%s-%s" % (spec.version, spec.compiler.name),
                    )
                )
            else:
                assert not os.path.exists(
...
@pytest.mark.parametrize(
    "method", [spack.cmd.env.env_activate, spack.cmd.env.env_deactivate]
)
@pytest.mark.parametrize(
    "env,no_env,env_dir",
    [
...
def test_activate_temp(monkeypatch, tmpdir):
    ...
    active_env_var = next(
        line for line in shell.splitlines() if ev.spack_env_var in line
    )
