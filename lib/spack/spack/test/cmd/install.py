...
def test_install_env_variables(
    ...
    spec.concretize()
    install('libdwarf')
    assert spec.package.install_env_path.is_file()
...
def test_install_env_with_tests_root(tmpdir, mock_packages, mock_fetch,
    ...
    with ev.read('test'):
        ...
        add('depb')
        install('--test', 'root')
        assert not test_dep.prefix.exists()
