...
def test_manpath_trailing_colon(
    shell, set_command, command_end, install_mockery, mock_fetch, mock_archive, mock_packages, working_env
):
    ...
    lines = bat_out.split("\n")
    assert any(re.match(set_command % ("MANPATH", ".*" + os.pathsep + command_end), ln) for ln in lines)
    ...
    lines = bat_out.split("\n")
    assert any(
        re.match(set_command % ("MANPATH", ".*" + os.pathsep + "/tmp/man" + os.pathsep + command_end ), ln)
        for ln in lines
    )
