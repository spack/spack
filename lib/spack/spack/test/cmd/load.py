...
@pytest.mark.parametrize(
    ...
    [("--bat", 'set "%s=%s"', "")] if sys.platform == "win32" else [("--sh", "export %s=%s", ";")],
)
def test_manpath_trailing_colon(
    ...
    lines = bat_out.split("\n")
    assert any(
        re.match(set_command % ("MANPATH", ".*" + os.pathsep + command_end), ln) for ln in lines
    )
    ...
    assert any(
        re.match(
            set_command % ("MANPATH", ".*" + os.pathsep + "/tmp/man" + os.pathsep + command_end),
            ln,
        )
        for ln in lines
    )
