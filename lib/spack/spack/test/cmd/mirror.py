...
def test_mirror_from_env(
    tmpdir, mock_packages, mock_fetch, config, mutable_mock_env_path
):
...
def test_mirror_skip_unstable(
    tmpdir_factory, mock_packages, config, source_for_pkg_with_hash
):
    mirror_dir = str(tmpdir_factory.mktemp("mirror-dir"))

    specs = [
        spack.spec.Spec(x).concretized()
...
def test_exclude_specs(mock_packages, config):
    ...
    expected_include = set(
        spack.spec.Spec(x) for x in ["mpich@3.0.3", "mpich@3.0.4", "mpich@3.0"]
    )
    expected_exclude = set(
        spack.spec.Spec(x) for x in ["mpich@3.0.1", "mpich@3.0.2", "mpich@1.0"]
    )
...
"""
    ...
    args = MockMirrorArgs(
        specs=["mpich"], versions_per_spec="all", exclude_file=exclude_path
    )
...
def test_mirror_crud(tmp_scope, capsys):
    with capsys.disabled():
        ...
        output = mirror(
            "set-url", "--scope", tmp_scope, "--push", "mirror", "s3://spack-public"
        )
