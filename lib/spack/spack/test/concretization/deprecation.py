...
packages:
  all:
    allowed_deprecation_level: critical
""")
...
def test_allowed_deprecation_level_per_package_blocks(
    mock_packages, concretize_scope, packages_yaml_write
):
    """Tests that when the allowed_deprecation_level < severity, concretization fails."""
    packages_yaml_write("""
packages:
  deprecated-with-reason:
    allowed_deprecation_level: medium
""")
    with pytest.raises(UnsatisfiableSpecError, match="deprecated"):
        concretize_one("deprecated-with-reason@1.0")


def test_allowed_deprecation_level_all_blocks(
    mock_packages, concretize_scope, packages_yaml_write
):
    """Tests that the all:allowed_deprecation_level applies when no per-package override exists."""
    packages_yaml_write("""
packages:
  all:
...
def test_allowed_deprecation_level_critical_config_warns_not_errors(
    mock_packages, concretize_scope, packages_yaml_write
):
