...
class TestConcretize(object):
    ...
    def test_architecture_deep_inheritance(self, mock_targets):
        ...
        with spack.repo.use_repositories(mock_repo):
            spec = Spec(
                "foopkg %gcc@4.5.0 os=CNL target=nocona"
    ...
    def test_virtual_is_fully_expanded_for_mpileaks(self):
        ...
        assert (
            "fake"
            in spec._dependencies["callpath"]
    ...
    @pytest.mark.parametrize(
        "compiler_str", ["clang", "gcc", "gcc@4.5.0", "clang@:3.3.0"]
    )
    ...
    def test_find_spec_parents(self):
        ...
        s = Spec.from_literal(
            {"a +foo": {"b +foo": {"c": None, "d+foo": None}, "e +foo": None}}
        )
    ...
    def test_find_spec_children(self):
        s = Spec.from_literal(
            {"a": {"b +foo": {"c": None, "d+foo": None}, "e +foo": None}}
        )

        assert "d" == find_spec(s["b"], lambda s: "+foo" in s).name

        s = Spec.from_literal(
            {"a": {"b +foo": {"c+foo": None, "d": None}, "e +foo": None}}
        )
    ...
    def test_find_spec_sibling(self):
        ...
        s = Spec.from_literal(
            {"a": {"b +foo": {"c": None, "d": None}, "e": {"f +foo": None}}}
        )
    ...
    def test_conditional_variants_fail(self, bad_spec):
        with pytest.raises(
            (spack.error.UnsatisfiableSpecError, vt.InvalidVariantForSpecError)
        ):
    ...
    def test_package_with_constraint_not_met_by_external(self):
        ...
        packages_yaml = {
            "libelf": {"externals": [{"spec": "libelf@0.8.13", "prefix": "/usr"}]}
        }
        spack.config.set("packages", packages_yaml)
    ...
    def test_reuse_installed_packages(
        ...
        new_root = Spec("root ^/{0}".format(dependency.dag_hash())).concretized()
    ...
    def test_mv_variants_disjoint_sets_from_packages_yaml(self):
        external_mvapich2 = {
            "mvapich2": {
                "buildable": False,
                "externals": [
                    {"spec": "mvapich2@2.3.1 file_systems=nfs,ufs", "prefix": "/usr"}
                ],
            }
        }
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            (
                ...
                {"libs=shared": False, "libs=static": True},
            ),
        ],
    )
