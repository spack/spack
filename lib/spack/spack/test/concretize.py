...
class TestConcretize(object):
    ...
    def test_concretize_separate_build(self):
        spec = check_concretize('separate-build-top')
        assert 'separate-build-build' in spec.build_only_deps
        assert spec.build_only_deps['separate-build-build'].satisfies(
            '^separate-build-link~X')
        assert spec.satisfies('^separate-build-link+X')

    def test_concretize_mention_run_dep(self):
        spec = check_concretize('dtbuild1 ^dtrun2@1.0')

        # Check parent's perspective of child
        dependency = spec.dependencies_dict()['dtrun2']
        assert set(dependency.deptypes) == set(['run'])

        # Check child's perspective of parent
        dtrun2 = spec['dtrun2']
        dependent = dtrun2.dependents_dict()['dtbuild1']
        assert set(dependent.deptypes) == set(['run'])
    ...
    def test_concretize_with_build_only_dep(self):
        ...
        assert (client_compiler.operating_system !=
    ...
    def test_concretize_link_dep_of_build_only_dep(self):
        ...
        spec = check_concretize('dttop platform=test os=be target=be')
        ...
        assert (dttop_compiler.operating_system !=
                dtlink2_compiler.operating_system)
        assert (dtbuild1_compiler.operating_system ==
                dtlink2_compiler.operating_system)
    ...
    def test_concretize_frontend_with_build_only_dep(self):
        ...
        spec = check_concretize('dttop platform=test os=fe target=fe')
        dttop_compiler = spack.compilers.compiler_for_spec(
            spec.compiler, spec.architecture)
        dtbuild1_spec = spec.build_only_deps['dtbuild1']
        assert spec.architecture == dtbuild1_spec.architecture
        dtbuild1_compiler = spack.compilers.compiler_for_spec(
            dtbuild1_spec.compiler, dtbuild1_spec.architecture)
        dtlink2_spec = dtbuild1_spec['dtlink2']
        dtlink2_compiler = spack.compilers.compiler_for_spec(
    ...
    def test_different_compilers_get_different_flags(self):
        top = Spec('dttop %gcc@4.7.2 platform=test os=fe target=fe' +
                   ' ^dtlink1 %clang@3.5 platform=test os=fe target=fe')
        top.concretize()
        dep = top['dtlink1']
        assert set(top.compiler_flags['cflags']) == set(['-O0'])
        assert set(dep.compiler_flags['cflags']) == set(['-O3'])
        assert set(top.compiler_flags['fflags']) == set(['-O0'])
        assert not set(dep.compiler_flags['fflags'])
