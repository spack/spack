...
def test_substitute_config_variables(mock_low_high_config, monkeypatch, tmp_path: pathlib.Path):
    ...
    assert os.path.join(spack_paths.prefix, "foo", "bar", "baz") == spack_path.canonicalize_path(
        "$spack/foo/bar/baz/"
    )
    ...
    if sys.platform != "win32":
        prefix = spack_paths.prefix.lstrip(os.sep)
        base = str(tmp_path)
...
def test_substitute_user_cache(mock_low_high_config):
    user_cache_path = spack_paths.user_cache_path
...
def test_license_dir_config(mutable_config, mock_packages, tmp_path):
    ...
    expected_dir = spack_paths.default_license_dir
