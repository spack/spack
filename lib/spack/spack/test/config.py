...
def check_compiler_config(comps, *compiler_names):
    ...
    for compiler in config:
        ...
        if conf["spec"] in compiler_names:
            comp = next(
                (c["compiler"] for c in comps if c["compiler"]["spec"] == conf["spec"]),
                None,
            )
            if not comp:
...
def test_substitute_config_variables(mock_low_high_config, monkeypatch):
    ...
    assert os.path.join("/foo/bar/baz", prefix) == spack_path.canonicalize_path(
        "/foo/bar/baz/$spack"
    )
    ...
    assert spack_path.canonicalize_path("/foo/bar/baz/$env") == "/foo/bar/baz/$env"
    ...
    assert spack_path.canonicalize_path("$env/foo/bar/baz") == os.path.join(
        fake_env_path, "foo/bar/baz"
    )

    # relative paths without source information are relative to cwd
    assert spack_path.canonicalize_path("foo/bar/baz") == os.path.abspath("foo/bar/baz")

    # relative paths with source information are relative to the file
    spack.config.set("modules:default", {"roots": {"lmod": "foo/bar/baz"}}, scope="low")
    spack.config.config.clear_caches()
    path = spack.config.get("modules:default:roots:lmod")
    assert spack_path.canonicalize_path(path) == os.path.normpath(
        os.path.join(mock_low_high_config.scopes["low"].path, "foo/bar/baz")
    )


packages_merge_low = {
    "packages": {"foo": {"variants": ["+v1"]}, "bar": {"variants": ["+v2"]}}
}
...
full_padded_string = os.path.join("/path", os.path.sep.join(reps))[:MAX_PADDED_LEN]
...
def test_nested_override():
    ...
    def _check_scopes(num_expected, debug_values):
        scope_names = [
            s.name
            for s in spack.config.config.scopes.values()
...
def test_dotkit_in_config_does_not_raise(
    ...
    write_config_file(
        "config", {"config": {"module_roots": {"dotkit": "/some/path"}}}, "high"
    )
...
def test_internal_config_section_override(mock_low_high_config, write_config_file):
    ...
    mock_low_high_config.push_scope(
        spack.config.InternalConfigScope(
            "high", {"config:": {"build_stage": wanted_list}}
        )
    )
...
def test_internal_config_dict_override(mock_low_high_config, write_config_file):
    ...
    mock_low_high_config.push_scope(
        spack.config.InternalConfigScope("high", config_override_dict)
    )
...
def test_internal_config_list_override(mock_low_high_config, write_config_file):
    ...
    wanted_list = config_override_list["config"]["build_stage:"]
    mock_low_high_config.push_scope(
        spack.config.InternalConfigScope("high", config_override_list)
    )
    assert mock_low_high_config.get("config:build_stage") == wanted_list
...
def test_set_dict_override(mock_low_high_config, write_config_file):
    ...
    assert config_merge_dict["config"]["info"] == mock_low_high_config.get(
        "config:info"
    )
