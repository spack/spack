...
"""Tests for llnl.util.url functions"""
import itertools
...
@pytest.fixture(params=llnl.util.url.ALLOWED_ARCHIVE_TYPES)
...
def test_get_extension(archive_and_expected):
    ...
    result = llnl.util.url.extension_from_path(archive)
    assert result == expected


def test_get_bad_extension():
    """Tests that a bad extension returns None"""
    result = llnl.util.url.extension_from_path("Foo.cxx")
...
def test_url_strip_version_suffixes(url, expected):
    stripped = llnl.util.url.strip_version_suffixes(url)
    assert stripped == expected


def test_strip_compression_extension(archive_and_expected):
    archive, extension = archive_and_expected
    stripped = llnl.util.url.strip_compression_extension(archive)
    if extension == "zip":
        assert stripped == "Foo.zip"
        stripped = llnl.util.url.strip_compression_extension(archive, "zip")
        assert stripped == "Foo"
    elif extension == "whl":
        assert stripped == "Foo.whl"
    elif (
        extension.lower() == "tar"
        or extension in llnl.util.url.CONTRACTION_MAP
        or extension
        in [
            ".".join(ext)
            for ext in itertools.product(llnl.util.url.PREFIX_EXTENSIONS, llnl.util.url.EXTENSIONS)
        ]
    ):
...
def test_allowed_archive(archive_and_expected):
    ...
    assert llnl.util.url.allowed_archive(archive)
