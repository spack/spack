...
"""
    ...
    assert (
        get_path_from_module_contents(module_show_lines, "cmake-3.9.2")
        == "/path/to/cmake-3.9.2"
    )
...
def test_pkg_dir_from_module_name():
    ...
    assert (
        get_path_from_module_contents(module_show_lines, "foo-bar")
        == "/path/to/foo-bar"
    )

    assert (
        get_path_from_module_contents(module_show_lines, "foo-bar/1.0")
        == "/path/to/foo-bar"
    )
...
def test_get_argument_from_module_line():
    ...
    assert all(
        get_path_args_from_module_line(x) == ["/lib/path", "/pkg/path"]
