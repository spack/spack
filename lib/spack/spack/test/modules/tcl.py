...
class TestTcl(object):
    ...
    def test_module_index(self, module_configuration, factory, tmpdir_factory):
        ...
        assert index[s1.dag_hash()].use_name == w1.layout.use_names[0]
        ...
        assert len(index) == 1
        assert index[s3.dag_hash()].use_name == w3.layout.use_names[0]
    ...
    def test_suffixes(self, module_configuration, factory):
        ...
        assert "foo" in writer.layout.use_names[0]
        assert "foo-foo" not in writer.layout.use_names[0]

        writer, spec = factory("mpileaks~debug target=x86_64")
        assert "foo-bar" in writer.layout.use_names[0]
        assert "baz" not in writer.layout.use_names[0]

        writer, spec = factory("mpileaks~debug+opt target=x86_64")
        assert "baz-foo-bar" in writer.layout.use_names[0]
    ...
    def test_override_config(self, module_configuration, factory):
        ...
        assert "mpich-static" in writer.layout.use_names[0]
        assert "over" not in writer.layout.use_names[0]
        assert "ridden" not in writer.layout.use_names[0]

        writer, spec = factory("mpileaks+opt target=x86_64")
        assert "over-ridden" in writer.layout.use_names[0]
        assert "mpich" not in writer.layout.use_names[0]
        assert "static" not in writer.layout.use_names[0]
