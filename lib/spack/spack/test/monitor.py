...
def get_client(host, prefix="ms1", allow_fail=False, tags=None, save_local=False):
    ...
    if not save_local:
        ...
        if info:
            tty.debug(
                "%s v.%s has status %s" % (info["id"], info["version"], info["status"])
            )
    return cli
...
def mock_monitor_request(monkeypatch):
    ...
    def mock_do_request(self, endpoint, *args, **kwargs):
        ...
        # Update Build
            ...
            return {"message": "Status updated", "data": {"build": build}, "code": 200}
        ...
        # Update Build Phase
            ...
            return {
                ...
                "code": 200,
                "data": {
                    "build_phase": {"id": 1, "status": "SUCCESS", "name": "autoconf"}
                },
            }
...
def test_spack_monitor_new_build(
    mock_monitor_request, install_mockery_mutable_config, install_mockery
):
    monitor = get_client(host="hostname")
    spec = spack.spec.Spec("dttop")
    spec.concretize()
...
def test_spack_monitor_update_build(
    mock_monitor_request, install_mockery, install_mockery_mutable_config
):
    monitor = get_client(host="hostname")
    spec = spack.spec.Spec("dttop")
    spec.concretize()
...
def test_spack_monitor_fail_task(
    mock_monitor_request, install_mockery, install_mockery_mutable_config
):
    monitor = get_client(host="hostname")
    spec = spack.spec.Spec("dttop")
...
def test_spack_monitor_send_analyze_metadata(
    monkeypatch, mock_monitor_request, install_mockery, install_mockery_mutable_config
):
...
def test_spack_monitor_send_phase(
    ...
    spec = spack.spec.Spec("dttop")
    spec.concretize()
    response = monitor.send_phase(
        spec.package, "autoconf", spec.package.install_log_path, "SUCCESS"
    )
    assert "message" in response and "data" in response and "code" in response
    assert response["code"] == 200
...
def test_install_monitor_save_local(
    install_mockery_mutable_config, mock_fetch, tmpdir_factory
):
    """
