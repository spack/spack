...
def test_packages_are_pickleable():
    ...
    if failed_to_pickle:
        tty.msg(
            "The following packages failed to pickle: " + ", ".join(failed_to_pickle)
        )
...
def test_packages_are_unparseable():
    ...
    if failed_to_unparse:
        tty.msg(
            "The following packages failed to unparse: " + ", ".join(failed_to_unparse)
        )
        assert False

    if failed_to_compile:
        tty.msg(
            "The following unparsed packages failed to compile: "
...
def test_api_for_build_and_run_environment():
    ...
    for pkg in spack.repo.path.all_packages():
        add_to_list = hasattr(pkg, "setup_environment") or hasattr(
...
def test_all_dependencies_exist():
    ...
    spack.package.possible_dependencies(*pkgs, transitive=True, missing=missing)
