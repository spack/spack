...
class TestPackage(object):
    ...
    def test_package_filename(self):
        ...
        assert filename == os.path.join(
            mock_packages_path, "packages", "mpich", "package.py"
        )
...
def test_url_for_version_with_only_overrides(mock_packages, config):
    ...
    assert (
        pkg.url_for_version("1.0.0") == "http://a.example.com/url_override-1.0.0.tar.gz"
    )
    assert (
        pkg.url_for_version("0.9.0") == "http://b.example.com/url_override-0.9.0.tar.gz"
    )
    assert (
        pkg.url_for_version("0.8.1") == "http://c.example.com/url_override-0.8.1.tar.gz"
    )

    # these don't exist but should still work, even if there are only overrides
    assert (
        pkg.url_for_version("1.0.5") == "http://a.example.com/url_override-1.0.5.tar.gz"
    )
    assert (
        pkg.url_for_version("0.9.5") == "http://b.example.com/url_override-0.9.5.tar.gz"
    )
    assert (
        pkg.url_for_version("0.8.5") == "http://c.example.com/url_override-0.8.5.tar.gz"
    )
    assert (
        pkg.url_for_version("0.7.0") == "http://c.example.com/url_override-0.7.0.tar.gz"
    )
