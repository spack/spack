...
def test_relocate_links(tmpdir):
    with tmpdir.as_cwd():
        ...
        relocate_links(
            filenames, old_layout_root, old_install_prefix, new_install_prefix
        )
...
def test_replace_paths(tmpdir):
    with tmpdir.as_cwd():
        ...
        assert out_dict == {
            ...
            os.path.join(oldlibdir_local, libfile_loco): os.path.join(
                libdir_local, libfile_loco
            ),
...
def test_manual_download(install_mockery, mock_download, monkeypatch, manual, instr):
    ...
    if instr:
        monkeypatch.setattr(spack.package.PackageBase, "download_instr", _instr)
