...
class TestSpecSematics(object):
    ...
    def test_satisfies_architecture(self):
        ...
        check_satisfies(
            "foo platform=test", "platform=test os=frontend target=frontend"
        )
        check_satisfies(
            "foo platform=test os=frontend target=frontend", "platform=test"
        )
    ...
    def test_satisfies_multi_value_variant(self):
        ...
        check_satisfies(
            'multivalue-variant foo="bar,baz"', 'multivalue-variant foo="baz"'
        )
    ...
    def test_errors_in_variant_directive(self):
        ...
        fn = variant(
            "foo", values=spack.variant.any_combination_of("fee", "foom"), default="bar"
        )
        with pytest.raises(spack.directives.DirectiveError) as exc_info:
            fn(Pkg())
        assert " it is handled by an attribute of the 'values' " "argument" in str(
            exc_info.value
        )
        ...
        with pytest.raises(spack.directives.DirectiveError) as exc_info:
            fn(Pkg())
        assert "either a default was not explicitly set, or 'None' was used" in str(
            exc_info.value
        )
    ...
    def test_splice(self, transitive):
        ...
        assert out["splice-t"].build_spec.full_hash() == spec["splice-t"].full_hash()
    ...
    def test_splice_subsequent(self, transitive):
        ...
        assert out2["splice-t"].build_spec.full_hash() == spec["splice-t"].full_hash()
        assert out2.spliced
    ...
    def test_splice_dict_roundtrip(self, transitive):
        ...
        # was transitive.
        expected_z_bld_hash = (
            dep["splice-z"].full_hash() if transitive else spec["splice-z"].full_hash()
        )
