...
def test_specfile_error_conditions_windows(text, exc_cls):
    with pytest.raises(exc_cls):
        SpecParser(text, Spec, specfiles=True).all_specs()
...
def test_parse_specfile_simple(specfile_for, tmp_path: pathlib.Path):
    ...
    spec = SpecParser(str(specfile), Spec, specfiles=True).next_spec()
    assert spec == s

    # Check we can mix literal and spec-file in text
    specs = SpecParser(f"mvapich_foo {str(specfile)}", Spec, specfiles=True).all_specs()
    assert len(specs) == 2
...
def test_parse_specfile_dependency(config, mock_packages, tmp_path: pathlib.Path):
    ...
    spec = SpecParser(f"libdwarf ^ {str(specfile)}", Spec, specfiles=True).next_spec()
    assert spec and spec["libelf"] == s["libelf"]

    with fs.working_dir(str(tmp_path)):
        # Make sure this also works: "spack spec ./libelf.yaml"
        spec = SpecParser(
            f"libdwarf^.{os.path.sep}{specfile.name}", Spec, specfiles=True
        ).next_spec()
        assert spec and spec["libelf"] == s["libelf"]
...
def test_parse_specfile_relative_paths(specfile_for, tmp_path: pathlib.Path):
    ...
    with fs.working_dir(str(parent_dir)):
        # Make sure this also works: "spack spec ./libelf.yaml"
        spec = SpecParser(f".{os.path.sep}{basename}", Spec, specfiles=True).next_spec()
        assert spec == s

        # Should also be accepted: "spack spec ../<cur-dir>/libelf.yaml"
        spec = SpecParser(
            f"..{os.path.sep}{parent_dir.name}{os.path.sep}{basename}", Spec, specfiles=True
        ).next_spec()
        assert spec == s
...
def test_parse_specfile_relative_subdir_path(specfile_for, tmp_path: pathlib.Path):
    ...
    with fs.working_dir(str(tmp_path)):
        spec = SpecParser(f"subdir{os.path.sep}{specfile.name}", Spec, specfiles=True).next_spec()
        assert spec == s
