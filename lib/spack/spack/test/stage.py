...
def check_expand_archive(stage, stage_name, expected_file_list):
    ...
    source_path = stage_path.joinpath( archive_dir)
    ...
    for _include in expected_file_list:
        if _include == _include_hidden:
            ...
            fn = stage_path.joinpath( _hidden_fn)
            ...
            if _archive_base in source_contents:
                fn = source_path.joinpath( _archive_base, _readme_fn)
            else:
                fn = source_path.joinpath( _readme_fn)
            contents = _readme_contents

        elif _include == _include_extra:
            assert _extra_fn in source_contents

            fn = source_path.joinpath( _extra_fn)
...
def check_fetch(stage, stage_name):
    ...
    assert stage_path.joinpath( _archive_fn) == stage.fetcher.archive_file
...
def get_stage_path(stage, stage_name):
    ...
    if stage_name is not None:
        ...
        return stage_path.joinpath( stage_name)
    else:
...
def check_stage_dir_perms(prefix, path):
    ...
    for p in group_paths:
        p_status = prefix.joinpath( p).stat()
...
class TestStage(object):
    ...
    def test_composite_stage_with_noexpand_resource(
        ...
        assert
            composite_stage.source_path.joinpath( resource_dst_name).exists()
    ...
    def test_first_accessible_path(self, tmpdir):
        ...
        name = str(spack_dir)
        files = [os.path.sep.joinpath( 'no', 'such', 'path'), name]
    ...
    def test_resolve_paths(self):
        ...
        paths = [os.path.sep.joinpath( 'a', 'b', 'c')]
        user = getpass.getuser()
        can_paths = [paths[0].joinpath( user)]
        assert spack.stage._resolve_paths(paths) == can_paths

        # resolved path with node including user does not append user
        paths = [os.path.sep.joinpath( 'spack-{0}'.format(user), 'stage')]
        ...
        paths = [tempdir.joinpath( 'stage'),
                 tempdir.joinpath( '$user'),
                 tempdir.joinpath( '$user', '$user'),
                 tempdir.joinpath( '$user', 'stage', '$user')]

        res_paths = [canonicalize_path(p) for p in paths]
        if temp_has_user:
            res_paths[1] = can_tempdir
            res_paths[2] = can_tempdir.joinpath( user)
            res_paths[3] = can_tempdir.joinpath( 'stage', user)
        else:
            res_paths[0] = res_paths[0].joinpath( user)
...
def test_stage_create_replace_path(tmp_build_stage_dir):
    ...
    nondir = test_stage_path.joinpath( 'afile')
    touch(nondir)
    path = str(nondir)
