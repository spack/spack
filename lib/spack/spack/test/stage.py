...
class TestStage(object):
    ...
    def test_no_search_mirror_only(self, failing_fetch_strategy, failing_search_fn):
        stage = Stage(
            failing_fetch_strategy, name=self.stage_name, search_fn=failing_search_fn
        )
        with stage:
            try:
    ...
    def test_search_if_default_fails(
        self, failing_fetch_strategy, search_fn, err_msg, expected
    ):
    ...
    def test_get_stage_root_bad_path(self, clear_stage_root):
        ...
        with spack.config.override("config:build_stage", "/no/such/path"):
            with pytest.raises(
                spack.stage.StageError, match="No accessible stage paths in"
            ):
