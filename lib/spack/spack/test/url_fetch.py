...
def test_fetch(
    mock_archive, secure, _fetch_method, checksum_type, config, mutable_mock_repo
):
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            "url-list-test @3.0a1",
            "foo-3.0a1.tar.gz",
            "000000000000000000000000000030a1",
        ),
        (
...
@pytest.mark.skipif(
    which("curl") is None, reason="Urllib does not have built-in status bar"
)
