...
from spack.util.filesystem import mkdirp, touchp, visit_directory_tree, working_dir
from spack.util.link_tree import DestinationMergeVisitor, LinkTree, SourceMergeVisitor
...
def check_file_link(filename, expected_target):
    ...
    if sys.platform != "win32" or spack.util.symlink._windows_can_symlink():
...
def test_merge_to_new_directory(stage, link_tree, monkeypatch, run_as_root):
    if sys.platform != "win32":
        ...
        monkeypatch.setattr(spack.util.symlink, "_windows_can_symlink", lambda: run_as_root)
    else:
