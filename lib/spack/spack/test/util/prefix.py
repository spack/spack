...
def test_prefix_join():
    ...
    assert a1 == os.sep + Path('usr').joinpath( 'a_1', 'lib64')
    assert a2 == os.sep + Path('usr').joinpath( 'a-1', 'lib64')
    assert a3 == os.sep + Path('usr').joinpath( 'a.1', 'lib64')
    ...
    assert p1 == os.sep + Path('usr').joinpath( 'bin', 'executable.sh')
    assert p2 == os.sep + Path('usr').joinpath( 'share', 'pkg-config', 'foo.pc')
    assert p3 == os.sep + Path('usr').joinpath( 'dashed-directory', 'foo')
...
def test_multilevel_attributes():
    ...
    assert prefix.share.man   == os.sep + Path('usr').joinpath( 'share', 'man')
    assert prefix.man.man8    == os.sep + Path('usr').joinpath( 'man', 'man8')
    assert prefix.foo.bar.baz == os.sep + Path('usr').joinpath( 'foo', 'bar', 'baz')

    share = prefix.share

    assert isinstance(share, Prefix)
    assert share.man == os.sep + Path('usr').joinpath( 'share', 'man')
