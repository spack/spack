...
class TestVariantMapTest:
    def test_set(self, map_class, value_class) -> None:
        # All three types of options are accepted, keyed by their own name
        a = map_class()
        a.set(make_bool(value_class, "foo", True))
        a.set(make_single(value_class, "bar", "baz"))
        a.set(make_multi(value_class, "foobar", ("a", "b", "c", "d", "e")))
        assert list(a) == ["foo", "bar", "foobar"]

        # An entry already under that name is replaced
        a.set(make_single(value_class, "foo", "bar"))
        assert a["foo"] == make_single(value_class, "foo", "bar")
    ...
    def test_copy(self, map_class, value_class) -> None:
        ...
        c = a.copy()
        assert a == c
        assert type(c) is map_class
        assert all(type(v) is value_class for v in c.values())

    def test_str(self, map_class, value_class) -> None:
        c = map_class()
        c.set(make_multi(value_class, "foo", ("bar", "baz")))
        c.set(make_single(value_class, "foobar", "fee"))
        c.set(make_single(value_class, "feebar", "foo"))
        c.set(make_bool(value_class, "shared", True))
...
def test_possible_values_unwraps_conditional_values(option_class, when, expected):
    vdef = option_class(
        "flavor",
        default="new",
        description="",
...
def test_possible_values_when_checked_by_a_validator(option_class):
    vdef = option_class("flavor", default="1", description="", values=int)
