...
def _system_untar(archive_file: str, remove_archive_file: bool = False) -> str:
    ...
    """
    archive_file_no_ext = llnl.util.url.strip_extension(archive_file)
...
def _py_bunzip(archive_file: str) -> str:
    ...
    decompressed_file = os.path.basename(
        llnl.util.url.strip_compression_extension(archive_file, "bz2")
    )
...
def _py_gunzip(archive_file: str) -> str:
    ...
    module"""
    decompressed_file = os.path.basename(
        llnl.util.url.strip_compression_extension(archive_file, "gz")
    )
    working_dir = os.getcwd()
...
def _system_gunzip(archive_file: str) -> str:
    ...
    archive_file_no_ext = llnl.util.url.strip_compression_extension(archive_file)
    if archive_file_no_ext == archive_file:
...
def _unzip(archive_file: str) -> str:
    ...
    unzip(archive_file)
    return os.path.basename(llnl.util.url.strip_extension(archive_file, extension="zip"))
...
def _py_lzma(archive_file: str) -> str:
    ...
    lzma module."""
    decompressed_file = os.path.basename(
        llnl.util.url.strip_compression_extension(archive_file, "xz")
    )
...
def _xz(archive_file):
    ...
    line tool."""
    decompressed_file = os.path.basename(
        llnl.util.url.strip_extension(archive_file, extension="xz")
    )
    working_dir = os.getcwd()
    destination_abspath = os.path.join(working_dir, decompressed_file)
    compressed_file = os.path.basename(archive_file)
...
def _system_7zip(archive_file):
    ...
    """
    outfile = os.path.basename(llnl.util.url.strip_compression_extension(archive_file))
    _7z = which("7z")
    if not _7z:
        raise CommandNotFoundError(
            "7z unavailable, unable to extract %s files. 7z can be installed via Spack"
            % llnl.util.url.extension_from_path(archive_file)
        )
...
def decompressor_for(path: str, extension: Optional[str] = None):
    ...
    if not extension or not llnl.util.url.allowed_archive(extension):
        raise CommandNotFoundError(
...
def decompressor_for_win(extension: str) -> Callable[[str], Any]:
    ...
    """
    extension = llnl.util.url.expand_contracted_extension(extension)
    extension_to_decompressor: Dict[str, Callable[[str], Any]] = {
    ...
    compression_extension = llnl.util.url.compression_ext_from_compressed_archive(extension)
    decompressor = (
    ...
    for file_type in SUPPORTED_FILETYPES:
        ...
        if decompress and isinstance(file_type, CompressedFileTypeInterface):
            ...
            if not uncompressed_ext:
                ...
                return llnl.util.url.extension_from_path(stream.name)
...
def _maybe_abbreviate_extension(path: str, extension: str) -> str:
    ...
    return abbr if llnl.util.url.has_extension(path, abbr) else extension
...
def extension_from_magic_numbers(path: str, decompress: bool = False) -> Optional[str]:
    ...
    return llnl.util.url.extension_from_path(path)
