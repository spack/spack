...
like those in spack.util.filesystem, but which require logic from spack.util
"""
...
from spack.util.symlink import islink, readlink, resolve_link_target_relative_to_the_link, symlink
...
def find_all_shared_libraries(root, recursive=False, runtime=True):
    ...
    See documentation for `spack.util.filesystem.find_libraries` for more information
    """
...
{str(self.base_modification_prefix)}"
    ...
    def _link(self, path: pathlib.Path, dest_dir: pathlib.Path):
        ...
        if not dest_file.exists() and dest_dir.exists() and not dest_file == path:
            ...
            except spack.util.symlink.AlreadyExistsError:
                report_already_linked()
