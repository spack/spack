...
class DeleteOnCloseFile(io.FileIO):
    def close(self):
        filename = self.name
        super().close()
...
class SSHConnection(object):
    ...
    def from_url(cls, url):
        ...
        if url.netloc not in cls._connections:
            cls._connections[url.netloc] = cls(url)
    ...
    def _default_ssh_args(self):
        ...
        if self.url.port:
            args.extend(["-p", str(self.url.port)])
        return args
    ...
    def read(self, remote_path):
        """Fetch a remote file and return its contents as a binary stream.

        The stream is backed by a temporary file that is unlinked when closed,
        so callers should use it as a context manager or close it explicitly.
        """
        fd, tmp_path = tempfile.mkstemp(dir=self.download_location)
        os.close(fd)
        try:
            self.fetch(remote_path, tmp_path)
        except Exception:
            os.remove(tmp_path)
            raise
        return DeleteOnCloseFile(tmp_path, "rb")
