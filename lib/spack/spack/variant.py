...
class VariantValue:
    ...
    __slots__ = ("name", "concrete", "type", "_values", "_key")
    ...
    def with_values(self, value: ValueType) -> "VariantValue":
        ...
        return VariantValue(self.type, self.name, value, concrete=self.concrete)
    ...
    def as_type(self, type: VariantType) -> "VariantValue":
        ...
        if type == self.type:
            return self
        return VariantValue(type, self.name, self._values, concrete=self.concrete)

    def as_concrete(self) -> "VariantValue":
        """This variant, marked concrete."""
        if self.concrete:
            return self
        return VariantValue(self.type, self.name, self._values, concrete=True)
...
def intern_variant_value(value: VariantValue) -> VariantValue:
    ...
    """
    key = (value.type, value.name, value.concrete, value._values)
