...
class MetaVersion(object):
    """Either a Version, VersionList or VersionRange"""

    def __lt__(self, other):
        return lt_function[(self.lt_type, other.lt_type)](self,other)

    def __eq__(self, other):
        return eq_function[(self.lt_type, other.lt_type)](self,other)

    lt_type = None
    @property
    def lt_type(self):
        return type(self).lt_type
...
# ---------------------------------------------------------
class Version(MetaVersion):
    """Base class...  Opaque inside, except self.string"""

    @property
    def string(self):
        """Returns the original string from which this version was parsed."""
        raise NotImplementedError()
    ...
    def __repr__(self):
        return '{0}({1})'.format(type(self).__name__, self.string)
...
# --------------------------------------------------------
sentinel_scheme = object()
class SentinelVersion(Version):
    """Used to implement a version that's either SMALLER than all Verions
    or LARGER than all versions.  Like NULL in C, it's compatible with any
    version scheme."""
    lt_type = LT_SENTINEL

    def __init__(self):
        self.scheme = sentinel_scheme
        self.index = index

    def __contains__(self, other):
        return False

smallest_sentinel = SentinelVersion(-1)
largest_sentinel = SentinelVersion(1)
# --------------------------------------------------------
class BaseVersion(Version):
    lt_type = LT_BASE

    def __init__(self, scheme, string):
        """scheme: VersionScheme
            The set of versions to which we belong."""
        self.scheme = scheme
        self._string = string
    ...
    def __contains__(self, other):
        if other.lt_type != LT_BASE:
            return False
        return self.scheme.contains_versions(self, other)

# --------------------------------------------------------
_VersionRange = collections.namedtuple('_VersionRange', ('start', 'end'))

@total_ordering
class VersionRange(_VersionRange):

    lt_type = LT_RANGE

    def __new__(self, start, end):
        # We don't have access to the VersionScheme...
        ...
        if start is None:
            start = smallest_sentinel
        if end is None:
            end = largest_sentinel

        if end < start:
            raise ValueError("Invalid Version range: %s" % self)

        return _VersionRange.__new__(VersionRange, (start, end))
...
#        return not (self == other)
    ...
    def __contains__(self, other):
        ...
        if (other.start < self.start) \
            and (other.start not in self.start):
            return False

        return (other.end < self.end) \
            or (other.end in self.end)
...
#            else:
    ...
    def add(self, version):
        if not instanceof(version, MetaVersion):
            raise TypeError("Can't add %s to VersionList" % type(version))

        if version.lt_type == LT_LIST:
            for v in version:
                self.add(v)
        else:    # Version or VersionRange
            # This normalizes single-value version ranges.
            concrete = version.concrete
            version = version if concrete is None else concrete

            i = bisect_left(self, version)

            i -= 1
            while i >= 0 and version.overlaps(self[i]):
    ...
    # TODO: Should use copy.copy instead
    # def copy(self):
...
def lt_sentinel_sentinel(a,b):
    return a.index < b.index

def lt_base_base(a,b):
    if a.scheme != b.scheme:
        raise TypeError('Cannot compare Versions belonging to different schemes: {0} and {1}'.format(a,b))
    return a.scheme.lt_versions(a,b)

def lt_list_list(a,b):
    return list.__lt__(a.versions, b.versions)
# -------------

def lt_sentinel_base(a,b):
    if a.index < 0:    # LOWEST
        return True
    else:    # HIGHEST
        return False

def lt_base_sentinel(a,b):
    if b.index > 0:    # HIGHEST
        return True
    else:    # LOWEST
        return False
# -------------
def lt_version_range(v,r):
    return tuple.__lt__((v,v), r)
def lt_range_version(r,v):
    return tuple.__lt__(r, (v,v))

def lt_xxx_list(v,r):
    return lt_list_list(VersionList(v), r)
def lt_list_xxx(r,v):
    return lt_list_list(r, VersionList(v))

lt_function = {
    (LT_SENTINEL, LT_SENTINEL) : lt_sentinel_sentinel,
    (LT_SENTINEL, LT_BASE) : lt_sentinel_base,
    (LT_BASE, LT_SENTINEL) : lt_base_sentinel,
    (LT_BASE, LT_BASE) : lt_base_base,

    (LT_SENTINEL, LT_RANGE) : lt_version_range,
    (LT_SENTINEL, LT_LIST) : lt_xxx_list,
    (LT_BASE, LT_RANGE) : lt_version_range,
    (LT_BASE, LT_LIST) : lt_xxx_list,
    (LT_RANGE, LT_SENTINEL) : lt_range_version,
    (LT_RANGE, LT_BASE) : lt_range_version,
    (LT_RANGE, LT_RANGE) : tuple.__lt__, 
    (LT_RANGE, LT_LIST) : lt_xxx_list,
    (LT_LIST, LT_SENTINEL) : lt_list_xxx,
    (LT_LIST, LT_BASE) : lt_list_xxx,
    (LT_LIST, LT_RANGE) : lt_list_xxx,
    (LT_LIST, LT_LIST) : lt_list_list,
}
# --------------------------------------------------------
def eq_sentinel_sentinel(a,b):
    return a.index == b.index

def eq_base_base(a,b):
    if a.scheme != b.scheme:
        raise TypeError('Cannot compare Versions belonging to different schemes: {0} and {1}'.format(a,b))
    return a.scheme.eq_versions(a,b)

def eq_list_list(a,b):
    return list.__eq__(a.versions, b.versions)
# --------------

def eq_version_range(v,r):
    return tuple.__eq__((v,v), r)
def eq_range_version(r,v):
    return tuple.__eq__(r, (v,v))

def eq_xxx_list(v,r):
    return eq_list_list(VersionList(v), r)
def eq_list_xxx(r,v):
    return eq_list_list(r, VersionList(v))

eq_function = {
    (LT_SENTINEL, LT_SENTINEL) : eq_sentinel_sentinel,
    (LT_SENTINEL, LT_BASE) : lambda a,b : False,
    (LT_BASE, LT_SENTINEL) : lambda a,b : False,
    (LT_BASE, LT_BASE) : eq_base_base,

    (LT_SENTINEL, LT_RANGE) : eq_version_range,
    (LT_SENTINEL, LT_LIST) : eq_xxx_list,
    (LT_BASE, LT_RANGE) : eq_version_range,
    (LT_BASE, LT_LIST) : eq_xxx_list,
    (LT_RANGE, LT_SENTINEL) : eq_range_version,
    (LT_RANGE, LT_BASE) : eq_range_version,
    (LT_RANGE, LT_RANGE) : tuple.__eq__,
    (LT_RANGE, LT_LIST) : eq_range_xxx,
    (LT_LIST, LT_SENTINEL) : eq_list_xxx,
    (LT_LIST, LT_BASE) : eq_list_xxx,
    (LT_LIST, LT_RANGE) : eq_list_xxx,
    (LT_LIST, LT_LIST) : eq_list_list,
}
# --------------------------------------------------------------
...
class VersionScheme(object):
    """Base class for package-specific ways to define versions."""
    ...
    def new_version(self, string):
        """Creates a new Version object."""
        raise NotImplementedError()

    def _new_meta_version(self, string):
        ...
        if ',' in string:
            return VersionList(
                [self.new_version(x) for x in string.split(',')])

        elif ':' in string:
            s, e = string.split(':')
            start = self.new_version(s) if s else None
            end = self.new_version(e) if e else None
            return VersionRange(start, end)

        else:
            return self.new_version(string)
    ...
    def ver(obj):
        ...
        if isinstance(obj, (list, tuple)):
            ...
            return self._new_meta_version(obj)
        elif isinstance(obj, (int, float)):
            return self._new_meta_version(str(obj))
...
# ==============================================================
# The Default VersionScheme

class DefaultVersionScheme(VersionScheme):
    """The version scheme used by most Spack packages; traditionally, this
    was the ONLY version scheme."""

    def lt_versions(self, selfa,other):
        ...
        if selfa.version == other.version:
            return False

        # First priority: anything < develop
        sdev = selfa._isdevelop()
        if sdev:
            return False    # source = develop, it can't be < anything

        # Now we know !sdev
        odev = other._isdevelop()
        if odev:
            return True    # src < dst

        # now we know neither selfa nor other isdevelop().

        # Principle: Non-numeric is less than numeric
        # (so numeric will always be preferred by default)
        if selfa.isnumeric():
            if other.isnumeric():
                return selfa._numeric_lt(other)
            else:    # selfa = numeric; other = non-numeric
                # Numeric > Non-numeric (always)
                return False
        else:
            if other.isnumeric():  # selfa = non-numeric, other = numeric
                # non-numeric < numeric (always)
                return True
            else:  # Both non-numeric
                # Maybe consider other ways to compare here...
                return selfa.string < other.string



    def eq_versions(self, a,b):
...
class DefaultVersion(Version):
    """Class to represent versions in DefaultVersionScheme"""
    ...
    def __init__(self, string):
        ...
        if DefaultVersion.VALID_VERSION.match(string) is None:
