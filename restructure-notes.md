# Branch state after the restructure (2026-08-21)

Status notes for `hs/test/property-based-tests` / `hs/test/spec-algebra-fuzz` (PR 52801).

## State

- The branch sits on upstream/develop with one open extraction PR at the base:
  `spec.py: round-trip abstract patches:=<checksum>` (PR 52928). Abstract variants print
  exactly, so abstract specs round-trip through `str()`; only concrete specs render their
  patch checksums as 7-character `patches=` prefixes.
...
- A fifth commit left the branch with the 08-21 rebase onto the merged #52916, superseded
  by PR 52928: `spec.py: pin what the patches format leaves out` (the pinned round-trip
...
- The old tip is kept at `backup-property-based-tests-20260821`; the pre-rebase tip of
  08-21 at `property-rebase-backup-20260821`.
- `hs/test/spec-algebra-fuzz` (the PR head) points at the last code commit, now the
  split-constrain commit: `spec_algebra.py`, the harness scripts under
  `lib/spack/spack/test/` and these docs stay out of the PR.
- `formal/spec-semantics.tex` cites source line numbers, which the 08-20 rebase shifted by
  ...
  ∅-denoting concrete prefix states: the disjoint round-trip is gone with PR 52928 at the
  base, and the intersects asymmetry it listed does not reproduce on the branch (#52852
