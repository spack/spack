...
class Abacus(MakefilePackage):
    ...
    version('2.2.1', '14feca1d8d1ce025d3f263b85ebfbebc1a1efff704b6490e95b07603c55c1d63', url='https://github.com/abacusmodeling/abacus-develop/archive/refs/tags/v2.2.1.tar.gz')

    version('2.2.0', '09d4a2508d903121d29813a85791eeb3a905acbe1c5664b8a88903f8eda64b8f', url='https://github.com/abacusmodeling/abacus-develop/archive/refs/tags/v2.2.0.tar.gz')
    ...
    def edit(self, spec, prefix):
        if self.version == Version('2.2.1'):
            ...
            makefile_vars.filter('^\s*LAPACK_DIR\s*=.*',  'LAPACK_DIR = '+ spec['mkl'].prefix)
            makefile_vars.filter('^\s*FFTW_DIR\s*=.*',  'FFTW_DIR = '+ spec['fftw'].prefix)
            makefile_vars.filter('^\s*LIBXC_DIR\s*=.*',  'LIBXC_DIR = '+ spec['libxc'].prefix)
            makefile_vars.filter('^\s*CEREAL_DIR\s*=.*',  'CEREAL_DIR = '+ spec['cereal'].prefix)
            makefile_vars.filter('^\s*ELPA_DIR\s*=.*',  'ELPA_DIR = '+ spec['elpa'].prefix)
            
            makefile_system = FileFilter('source/Makefile.system')
            makefile_system.filter('^\s*ELPA_INCLUDE_DIR\s*=.*',  'ELPA_INCLUDE_DIR = -I${ELPA_DIR}/include/'+'elpa_openmp-{0}'.format(spec['elpa'].version))
        ...
        if self.version == Version('2.2.0'):
            ...
            makefile_vars.filter('^\s*CEREAL_DIR\s*=.*',  'CEREAL_DIR = '+ spec['cereal'].prefix)
            makefile_vars.filter('^\s*ELPA_DIR\s*=.*',  'ELPA_DIR = '+ spec['elpa'].prefix)
            makefile_vars.filter('^\s*ELPA_INCLUDE\s*=.*',  'ELPA_INCLUDE = -I${ELPA_DIR}/include/'+'elpa_openmp-{0}'.format(spec['elpa'].version))
