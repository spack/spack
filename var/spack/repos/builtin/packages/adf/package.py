...
class Adf(Package):
    ...
    electronic structure calculations that makes use of density functional theory."""
    ...
    def url_for_version(self, version):
        return "file://{0}/adf/adf{1}.pc64_linux.openmpi.bin.tgz".format(os.getcwd(),
                                                                         version)
    ...
    def install(self, spec, prefix):
        for filename in os.listdir(self.stage.source_path):
            ...
            elif not fnmatch.fnmatch(filename, "spack-build.*") or fnmatch.fnmatch(filename, "adf*"):
