...
class Babelstream(CMakePackage, CudaPackage, ROCmPackage):
    ...
    conflicts("dir=none", when="+kokkos",
              msg="KOKKKOS requires architecture to be specfied by dir=")
    variant("kokkos", default=False, description="Enable KOKKOS support")

    # ACC conflict
    variant("cpu_arch", values=str, default="none",
            description="Enable CPU Target for ACC")
    variant("target", values=str, default="none",
            description="Enable CPU Target for ACC")

    # STD conflicts
    conflicts("+stddata", when="%gcc@:10.1.0",
              msg="STD-data requires newer version of GCC")
    conflicts("+stdindices", when="%gcc@:10.1.0",
              msg="STD-indices requires newer version of GCC")
    conflicts("+stdranges", when="%gcc@:10.1.0",
              msg="STD-ranges requires newer version of GCC")

    # CUDA conflict
    conflicts("cuda_arch=none", when="+cuda",
              msg="CUDA requires architecture to be specfied by cuda_arch=")
    variant("mem", values=str, default="DEFAULT",
            description="Enable MEM Target for CUDA")
    # Raja Conflict
    variant("offload", values=str, default="none",
            description="Enable RAJA Target [CPU or NVIDIA]")
    conflicts("offload=none", when="+raja",
              msg="RAJA requires architecture to be specfied by target=[CPU,NVIDIA]")

    # download raja from https://github.com/LLNL/RAJA
    conflicts("dir=none", when="+raja",
              msg="RAJA implementation requires architecture to be specfied by dir= [#download raja from https://github.com/LLNL/RAJA]")
    ...
    variant("partitioner", values=partitioner_vals, default="auto", description="Partitioner specifies how a loop template should partition its work among threads.\
            Possible values are:\
    ...
    variant("backend", values=backend_vals, default="none",
    ...
    variant("flags", values=str, default="none",
            description="Additional CXX flags to be provided")

    # comp_impl_vals=["ONEAPI-DPCPP","DPCPP","HIPSYCL","COMPUTECPP"]
    variant("implementation", values=str, default="none",
            description="Compile using the specified SYCL compiler option")

    conflicts("implementation=none", when="+sycl",
              msg="SYCL requires compiler implementation to be specified by option=")
    conflicts("implementation=none", when="+thrust", msg="Which Thrust implementation to use, supported options include:\
    ...
    def cmake_args(self):
        ...
        model_variants_list = find_model_flag(
            spec_string_truncate)  # Prints out ['cuda', 'thrust']

        if (len(model_variants_list) > 1):
            ignore_list = ["cuda"]  # if +acc is provided ignore the cuda model
            model = list(set(model_variants_list) - set(ignore_list))
            # We choose 'thrust' from the list of ['cuda', 'thrust']
            args = ["-DMODEL=" + model[0]]
        else:
            # if it is +stddata,indices etc. we need to pass it as std-data to the CMake compiler
            # do some alterations here
            if ("std" in model_variants_list[0]):
                args = ["-DMODEL=" + "std-" +
                        model_variants_list[0].split('d', 1)[1]]
            else:
                args = ["-DMODEL=" + model_variants_list[0]]

        # TODO: do we need to supply this for all ? since we don't have this flag for some of languages
        # args.append("-DCXX_EXTRA_FLAGS=" + self.spec.variants["flags"].value + " -O3")
        ...
        if ("+acc" in self.spec) and ("~cuda" in self.spec):
            ...
            if ("cuda_arch" in self.spec.variants):
                ...
                args.append("-DTARGET_PROCESSOR=" +
                            self.spec.variants['cpu_arch'].value)

            # TODO:  include an option to choose the target and the related architecture
            #       right now we have 2 automatic ones but it could be different too

        # ===================================
        #    STDdata,STDindices,STDranges
        # ===================================
        if ("+stddata" in self.spec) or ("+stdindices" in self.spec) or ("+stdranges" in self.spec):
            args.append("-DCMAKE_CXX_COMPILER=" + self.compiler.cxx)
        ...
        # cmake -Bbuild -H. -DMODEL=sycl2020 -DSYCL_COMPILER=ONEAPI-ICPX -DSYCL_COMPILER_DIR=ONEAPI-ICPX
        if "+sycl2020" in self.spec:
            if (self.spec.satisfies("%oneapi")):
                # -fsycl flag is required for setting up sycl/sycl.hpp seems like it doesn't get it from the CMake file
        ...
        if "+ocl" in self.spec:
            if ("backend" in self.spec.variants):
                if ("cuda" in self.spec.variants['backend'].value):
                    ...
                    intel_lib = self.spec["intel-oneapi-compilers"].prefix + \
        ...
        if bool(set(model_variants_list).intersection(["kokkos", "raja", "acc", "hip"]) == False):
