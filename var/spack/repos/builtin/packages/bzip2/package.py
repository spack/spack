...
class Bzip2(Package, SourcewarePackage):
    ...
    variant(
        "debug",
        default=False,
        description="Enable debug symbols and disable optimization",
    )
    ...
    def determine_version(cls, exe):
        ...
        match = re.search(
            r"bzip2, a block-sorting file compressor." "  Version ([^,]+)", output
        )
    ...
    def patch(self):
        ...
        if "darwin" in self.spec.architecture:
            ...
            mf.filter(
                "$(CC) -shared -Wl,-soname -Wl,libbz2.so.{0} -o libbz2.so.{1} $(OBJS)".format(  # noqa
                    v2, v3
                ),
                "$(CC) -dynamiclib -Wl,-install_name -Wl,@rpath/libbz2.{0}.dylib -current_version {1} -compatibility_version {2} -o libbz2.{3}.dylib $(OBJS)".format(  # noqa
                    v1, v2, v3, v3
                ),
                **kwargs
            )
