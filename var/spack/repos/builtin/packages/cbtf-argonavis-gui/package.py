...
class CbtfArgonavisGui(QMakePackage):
    ...
    depends_on("openspeedshop-utils+cuda@develop gui='none'", when='@develop')
    depends_on("openspeedshop-utils@2.3.1.3+cuda gui='none'", when='@1.3.0.0')
    ...
    depends_on("cbtf-argonavis@1.9.1.0", when='@1.3.0.0+cbtf+cuda')
