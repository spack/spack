    ...
    def edit(self, spec, prefix):
        ...
        if "+mpi" in self.spec:
            ...
            if "+elsi" in self.spec:
                ...
                march.filter(
                    "ELSIINCDIR .*",
                    "ELSIINCDIR = {0}".format(spec["elsi"].prefix.include),
                )
        ...
        if "+arpack" in self.spec:
            march.filter(
                "ARPACK_LIBS = .*",
                "ARPACK_LIBS = {0}".format(spec["arpack-ng"].libs.ld_flags),
            )
        ...
        if "+dftd3" in self.spec:
            ...
            march.filter(
                "DFTD3_INCS = .*",
                "DFTD3_INCS = -I{0}".format(spec["dftd3-lib"].prefix.include),
            )

            march.filter(
                "DFTD3_LIBS = .*",
                "DFTD3_LIBS = -L{0} -ldftd3".format(spec["dftd3-lib"].prefix),
            )
