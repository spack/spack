...
class Fastani(CMakePackage, AutotoolsPackage):
    ...
    build_system(
        conditional("cmake", when="@1.34:"),
        conditional("autotools", when="@1.33"),
        default="cmake",
    )

    depends_on("autoconf", type="build", when="@1.33")
    depends_on("automake", type="build", when="@1.33")
    depends_on("cmake@3.20:", type="build", when="@1.34:")
    depends_on("libtool", type="build", when="@1.33")
    depends_on("m4", type="build", when="@1.33")
