...
class Flamemaster(CMakePackage):
    ...
    variant('bilin_omega',  default=True,
            description='Compile with bilinear interpolation for collision integrals (omega)')
    variant('combustion',default=False,
            description='Integrate comustion libraries for kinetics, thermodynamics, and transport')
    variant('fortran_code', default=False,
            description='Compile optional fortran code like dassl, the adiabatic flame temperature module, and a mechanism converter tool')
    variant('scanmannew',  default=False, 
            description='Compile the new ScanMan')
    variant('dco_chunk_tape', default=False,
            description='Dynamically allocate memory for the dco tape')
    variant('troe', default=False,
            description='FlameMaster mechanism format allows for specifying an additional parameter. This option disables the use of the paramter which is currently required to run adjoint sensitivity analysis.')
    variant('jl_debug', default=False,
            description='Debug mode during development(Johannes Lotz)')
    variant('flux_bc', default=False,
	    description='Specify boundary conditions as velocity [m/s] or massflux in Counterflow (Physical Coord.)')
    variant('full_diffusion', default=False,
	    description='Compile with multi-component diffusion')
    variant('intel_prof', default=False,
            description='Enable -prof-gen and -prof-use')
    variant('soot_lib', default=False,
	    description='Soot library with moment equation solving')
    variant('newton_perf', default=False,
	    description='Compile with time measurement of each Newton step')
    variant('optima', default=False,
            description='Include code for Optima++ features')
    variant('opt_reaction_rates', default=True,
            description='Fast computation of reaction rates.')
    variant('prem_upwind_convec', default=True,
            description='Use upwind scheme for the convective term.')
    variant('sundials_hint', default=False,
            description='No hints on how FlameMaster can install sundials.')
    variant('simd_kinetics', default=False,
            description='WITH SIMD_KINETICS')
    variant('solve_mom_log', default=False,
            description='Solve the soot Moments equation with logarithms')
    variant('solve_with_z', default=True,
            description='Counter flow diffusion flame implementation includes a solution of the mixture fraction equation')
    variant('sundials_lapack', default=False,
	    description='Link LAPACK in sundials')
    variant('sundials_no_idas_search', default=False,
	    description='Do not search for IDAS (part of sundials).')
    variant('sundials_use_static_lib', default=True,
	    description='Use static linking for sundials libraries')
    variant('sundials_diagnos', default=False,
            description=' Only for troubleshooting')
    variant('tests', default=False,
            description= 'Install google-test framework for unit tests and enable units tests.')
    variant('third_party_in_build_dir', default=True,
	    description= 'Compile third party libraries and tools in build diretory')
    variant('local_lewis', default=False,
            description= 'Write Lewis numbers at every grid point')
    variant('flamemaster_prefix', default=True,
    ...
    def setup_build_environment(self, env):
        ...
        if self.spec.satisfies('%aocc'):
            env.append_flags('LDFLAGS', '-L{0} -lalm -lm -lstdc++'.format(pjoin(os.path.dirname(os.path.dirname(self.compiler.cxx)), "lib")))
    ...
    def cmake_args(self):
        ...
        if spec.variants['build_type'].value == 'Release':
            ...
            if self.spec.satisfies('%icc'):
                cxxflags = "-Ofast -ffast-math -DNDEBUG -march=native -mtune=native -funroll-all-loops -qopt-multi-version-aggressive -ipo -parallel"
                cflags = "-Ofast -ffast-math -DNDEBUG -march=native -mtune=native -funroll-all-loops -ipo -parallel"
...
###            ]
        ...
        args.append(self.define_from_variant('SUNDIALS_USE_STATIC_LIBRARIES','sundials_use_static_lib'))
        args.append(self.define_from_variant('TESTS','tests'))
        args.append(self.define_from_variant('THIRD_PARTY_IN_BUILD_DIR','third_party_in_build_dir'))
