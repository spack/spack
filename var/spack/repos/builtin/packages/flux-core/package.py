...
class FluxCore(AutotoolsPackage):
    ...
    depends_on("py-ply", when="@0.46.1:", type=("build", "run"))
    depends_on("jansson")
    ...
    depends_on("asciidoc", when="+docs", type="build")
    depends_on("py-docutils", when="@0.32.0: +docs", type="build")
    ...
    depends_on("flux-security", when="+security", type="build")
