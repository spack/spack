...
class Fsl(Package, CudaPackage):
    ...
    version(
        "6.0.5",
        sha256="df12b0b1161a26470ddf04e4c5d5d81580a04493890226207667ed8fd2b4b83f",
    )
    version(
        "6.0.4",
        sha256="58b88f38e080b05d70724d57342f58e1baf56e2bd3b98506a72b4446cad5033e",
    )
    version(
        "5.0.10",
        sha256="ca183e489320de0e502a7ba63230a7f55098917a519e8c738b005d526e700842",
    )
    ...
    def patch(self):
        ...
        if "+cuda" in self.spec:
            ...
            build_settings.filter(
                r"(^LIB_CUDA)\s*=.*",
                r"\1 = {0}".format(join_path(cuda_installation, "lib64")),
            )
            build_settings.filter(
                r"(^INC_CUDA)\s*=.*",
                r"\1 = {0}".format(join_path(cuda_installation, "include")),
            )
            build_settings.filter(
                r"(^NVCC11)\s*=.*",
