...
class Gate(CMakePackage):
    ...
    version(
        "9.0", sha256="8354f392facc0b7ae2ddf0eed61cc43136195b198ba399df25e874886b8b69cb"
    )

    variant(
        "rtk", default=False, description="build support for the Reconstruction Toolkit"
    )
    variant(
        "default_platform",
        default="condor",
    ...
    def setup_run_environment(self, env):
        ...
        env.set(
            "GC_CONDOR_SCRIPT",
            join_path(self.prefix, "share", "jobsplitter", "condor.script"),
        )
        env.set(
            "GC_PBS_SCRIPT",
            join_path(self.prefix, "share", "jobsplitter", "openPBS.script"),
        )
        env.set(
            "GC_SGE_SCRIPT",
            join_path(self.prefix, "share", "jobsplitter", "SGE.script"),
        )
        env.set(
            "GC_SLURM_SCRIPT",
            join_path(self.prefix, "share", "jobsplitter", "slurm.script"),
        )
