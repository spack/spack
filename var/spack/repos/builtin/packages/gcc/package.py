...
class Gcc(AutotoolsPackage, GNUMirrorPackage):
    ...
    depends_on(
        "binutils+gas+ld+plugins~libiberty",
        when="+binutils",
        type=("build", "link", "run"),
    )
    ...
    patch(
        ...
        sha256="b48e48736062e64a6da7cbe7e21a6c1c89422d1f49ef547c73b479a3f3f4935f",
        when="@6.5.0,7.4.0:7.5.0,8.2.0:9.3.0",
    )
    patch(
        "https://gcc.gnu.org/git/?p=gcc.git;a=patch;h=745dae5923aba02982563481d75a21595df22ff8",
        sha256="eaa00c91e08a5e767f023911a49bc1b2d1a3eea38703b745ab260f90e8da41aa",
        when="@10.1.0:11.1.0",
    )
    ...
    patch(
        ...
        sha256="28c5ab3b564d83dd7e6e35b9c683141a4cb57ee886c5367e54a0828538b3c789",
        when="@10.1:10.3",
    )
    patch(
        "https://gcc.gnu.org/git/?p=gcc.git;a=patch;h=f1feb74046e0feb0596b93bbb822fae02940a90e",
        sha256="3e5029489b79fc0d47fd6719f3d5c9d3bbc727a4a0cbff161a5517e8a3c98cb6",
        when="@11.1",
    )
