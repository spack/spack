...
from llnl.util.symlink import symlink, readlink
...
class Gcc(AutotoolsPackage, GNUMirrorPackage, CompilerPackage):
    ...
    def write_specs_file(self):
        ...
        with open(specs_file, "w") as f:
            builtin = self.spec["gcc"].command("-dumpspecs", output=str, error=os.devnull).strip()
            # if add_sysroot:
            #     # prepend the spack_glibc prefix to get the right loader
            #     # do we need a version of this? line = re.sub(r"/lib64/", "/lib/", line)
            #     # builtin = re.sub(r":-dynamic-linker %", f":-dynamic-linker %(spack_glibc)%", builtin)
