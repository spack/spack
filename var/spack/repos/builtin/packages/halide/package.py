...
class Halide(CMakePackage, PythonExtension):
    """Halide is a programming language designed to make it easier to write high-performance image
    and array processing code on modern machines. """
    ...
    depends_on("llvm@14.0.0:14+clang+lld targets=arm,x86,nvptx,aarch64,hexagon,webassembly build_type=Release", type=("link", "run"))
    ...
    def cmake_args(self):
        ...
        args = [ self.define("Python3_EXECUTABLE", spec["python"].command.path),
        ...
        if 'python' in spec:
            ...
            prefix = spec.prefix
            args += [self.define("Halide_INSTALL_PYTHONDIR", prefix + "/lib/python{0}/site-packages/".format(pyspec))]
        return args
