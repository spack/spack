...
class Hipsparselt(CMakePackage, ROCmPackage):
    ...
    for ver in ["6.0.0", "6.0.2", "master"]:
        depends_on(f"hip@{ver}", when=f"@{ver}")
        depends_on(f"hipsparse@{ver}", when=f"@{ver}")
        depends_on(f"rocm-openmp-extras@{ver}", when=f"@{ver}", type="test")

    depends_on("cmake@3.5:", type="build")
    depends_on(f"msgpack-c@3:")
    depends_on(f"python@3.6:")
    depends_on(f"py-virtualenv")
    depends_on(f"py-wheel")
    depends_on(f"py-pip")
    depends_on(f"py-pyyaml", type="test")
    depends_on(f"py-joblib")
