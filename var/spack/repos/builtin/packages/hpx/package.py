...
class Hpx(CMakePackage, CudaPackage):
    ...
    variant('std_tuple', default='ON', description="Enable `std::tuple' adaption")
    ...
    def cmake_args(self):
        ...
        # adapt std::tuple
        args.append('-DHPX_DATASTRUCTURES_WITH_ADAPT_STD_TUPLE={0}'.format(
            'ON' if 'std_tuple' in spec else 'OFF'
        ))
