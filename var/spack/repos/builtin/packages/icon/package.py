...
class Icon(AutotoolsPackage, CudaPackage):
    ...
    variant('rte-rrtmgp', default=True, description='Enable usage of the RTE+RRTMGP toolbox for radiation calculations')
    ...
    variant('claw', default='none', values=('none', ) + claw_values, description='Enable CLAW preprocessing')
    ...
    def configure_args(self):
        ...
        if self.compiler.name == 'gcc':
            ...
            config_vars['CFLAGS'].extend(['-g', '-gdwarf-4', '-O3', '-qno-opt-dynamic-align', '-ftz'])
        ...
        if '~gpu' in self.spec:
            ...
            config_vars['NVCFLAGS'].extend([
                '-g', '-O3', '-ccbin {0}'.format(cuda_host_compiler),
        ...
        if '+rttov' in self.spec:
            ...
            if rttov_major_version != ver(13):
                filter_file('_RTTOV_VERSION=13',
                            ...
                            'icon.mk', string=True, backup=False)
