...
class Intelmpi(IntelInstaller):
    ...
    def setup_dependent_environment(self, spack_env, run_env, dependent_spec):
        spack_env.set('MPICC',  join_path(self.prefix, 'impi', self.version, 
                                          'intel64', 'bin', 'mpicc'))
        spack_env.set('MPICXX', join_path(self.prefix, 'impi', self.version,
                                          'intel64', 'bin', 'mpicxx'))
        spack_env.set('MPIF77', join_path(self.prefix, 'impi', self.version,
                                          'intel64', 'bin', 'mpif77'))
        spack_env.set('MPIF90', join_path(self.prefix, 'impi', self.version,
                                          'intel64', 'bin', 'mpif90'))
    ...
    def setup_dependent_package(self, module, dep_spec):
        self.spec.mpicc = join_path(self.prefix, 'impi', self.version,
                                    'intel64', 'bin', 'mpicc')
        self.spec.mpicxx = join_path(self.prefix, 'impi', self.version,
                                     'intel64', 'bin', 'mpicxx')
        self.spec.mpifc = join_path(self.prefix, 'impi', self.version,
                                    'intel64', 'bin', 'mpif90')
        self.spec.mpif77 = join_path(self.prefix, 'impi', self.version,
                                     'intel64', 'bin', 'mpif77')
