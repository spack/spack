...
class Lbann(CMakePackage, CudaPackage, ROCmPackage):
    ...
    variant(
        "extras",
        default=False,
        description="Add python modules for LBANN related tools",
    )
    ...
    conflicts(
        "+extras", when="~pfe", msg="Python extras require the Python front end support"
    )
    ...
    for arch in CudaPackage.cuda_arch_values:
        ...
        depends_on(
            "dihydrogen cuda_arch=%s" % arch,
            when="+dihydrogen +cuda cuda_arch=%s" % arch,
        )
    ...
    for val in ROCmPackage.amdgpu_targets:
        ...
        depends_on(
            "dihydrogen amdgpu_target=%s" % val,
            when="+dihydrogen amdgpu_target=%s" % val,
        )
    ...
    depends_on(
        "opencv@4.1.0: build_type=RelWithDebInfo +core +highgui "
    ...
    depends_on(
        "py-numpy@1.16.0:", type=("build", "run"), when="@:0.90,0.99: +pfe +extras"
    )
