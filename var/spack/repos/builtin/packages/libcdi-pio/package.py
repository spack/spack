...
class LibcdiPio(AutotoolsPackage):
    ...
    variant("szip-grib1", default=False, description="Enable szip compression for GRIB1")
    variant("fortran", default=True, description="Enable Fortran interfaces")
    variant("threads", default=True, description="Compile and link for multithreading")
    ...
    def libs(self):
        ...
        libs = find_libraries(lib_names, root=self.prefix, shared=shared, recursive=True)
    ...
    def configure_args(self):
        ...
        if self.spec.variants["grib2"].value == "eccodes":
            ...
            config_args.append("--with-grib_api=" + self.spec["grib-api"].prefix)
        ...
        if "+fortran" in self.spec:
            config_args.extend(["--enable-iso-c-interface", "--enable-cf-interface"])
        else:
            config_args.extend(["--disable-iso-c-interface", "--disable-cf-interface"])

        if "+mpi" in self.spec:
            config_args.extend(
                ["CC=" + self.spec["mpi"].mpicc, "FC=" + self.spec["mpi"].mpifc, "--enable-mpi"]
            )
        else:
            # Due to a bug in the configure script we have to avoid explicit
        ...
        if self.run_tests and "^openmpi" in self.spec:
            config_args.append(
                self.spec["openmpi"].format("MPI_LAUNCH={prefix.bin.mpirun} --oversubscribe")
            )
        ...
        config_args.extend(
            [
                "LDFLAGS={0}".format(
                    " ".join(["-L" + d for d in libs.directories if not is_system_path(d)])
                ),
    ...
    def skip_mpi_tests(self):
        ...
        with working_dir(self.build_directory):
            ...
            mpi_launch = config_status(
                "-q", "--file=-:{0}".format(tmp_filename), output=str, error=os.devnull
            ).strip()
    ...
    def patch_libtool(self):
        ...
        # Fortran compilers:
        if not (
            self.spec.satisfies("+fortran+shared") and self.compiler.name in ["nvhpc", "aocc"]
        ):
            return
        ...
        with working_dir(self.build_directory):
            ...
            filter_file(
                ...
                "lt_prog_compiler_pic_FC=' {0}'".format(self.compiler.fc_pic_flag),
                "./config.status",
                string=False,
                backup=False,
            )
