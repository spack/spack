...
class Libpressio(CMakePackage, CudaPackage):
    ...
    depends_on("py-mpi4py", type=("build", "link", "run"), when="@0.54.0:+mpi+python")
    depends_on("py-numcodecs", type="run", when="@0.54.0:+python")
    depends_on("doxygen+graphviz", type="build", when="+docs")
