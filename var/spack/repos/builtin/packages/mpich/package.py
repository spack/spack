...
'-Wl,-flat_namespace'.""",
    ...
    depends_on("automake@1.15:", when="@3.3:3.3 +hwloc", type="build")
    depends_on("libtool@2.4.4:", when="@3.3:3.3 +hwloc", type="build")
    depends_on("m4", when="@3.3:3.3 +hwloc", type="build"),
    depends_on("autoconf@2.67:", when="@3.3:3.3 +hwloc", type="build")
    ...
    def autoreconf(self, spec, prefix):
        ...
        if os.path.exists(self.configure_abs_path) and not spec.satisfies("@3.3:3.3 +hwloc"):
            return
