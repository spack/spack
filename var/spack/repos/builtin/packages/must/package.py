...
class Must(CMakePackage):
    ...
    As MPI calls are complex and usage errors common, this functionality is extremely helpful for
    application developers that want to develop correct MPI applications. This includes errors
    that already manifest – segmentation faults or incorrect results – as well as many errors
    that are not visible to the application developer or do not manifest on a certain system or
    MPI implementation.
    """
    ...
    # build variants
    variant("fortran", default=True, description="Build MUST with Fortran support.")
    variant(
        "callpath",
        default=False,
        description="Enable stacktraces with the stackwalker from dyninst.",
    )
    variant("backward", default=True, description="Enable stacktraces with backward-cpp.")
    variant(
        "graphviz",
        default=False,
        description="Automatically call graphviz dot tool to visualize deadlocks.",
    )
    variant("tests", default=False, description="Selects whether tests are built.")
    ...
    def check_fortran(self):
        ...
        if self.spec.satisfies("+fortran"):
            if is_no_fortran_compiler:
                raise InstallError(
                    "must+fortran requires Fortran compiler but no Fortran compiler found!"
                )
    ...
    def cmake_args(self):
        ...
        # WARNING: "-DMUST_C_COMPILER:STRING:STRING=..." does not overwrite internal variables
        #          "-DMUST_C_COMPILER=..." (without :STRING) seems to work.
