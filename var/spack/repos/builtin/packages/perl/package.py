...
class Perl(Package):  # Perl doesn't use Autotools, it should subclass Package
    ...
    def setup_build_environment(self, env):
        ...
        if spec.satisfies("os=bigsur"):
            ...
            if not match:
                tty.warn("Failed to detect macOS command line tools version: " + output)
            else:
                if Version(match.group(1)) < Version("12"):
                    tty.warn(
                        "Setting SYSTEM_VERSION_COMPAT=1 due to older "
    ...
    def command(self):
        ...
        for ver in ("", self.spec.version):
            path = os.path.join(self.prefix.bin, "{0}{1}".format(self.spec.name, ver))
