...
class PyAstroid(PythonPackage):
    ...
    version(
        "1.4.3",
        sha256="8e9ce4e925a17442cec085a7fce05478b99e482c13819c619b7cdd793777bf6b",
    )
    version(
        "1.4.2",
    ...
    depends_on(
        "py-lazy-object-proxy@1.4.0:1.4", when="@2.3.1:2.7.2", type=("build", "run")
    )
    ...
    depends_on(
        "py-backports-functools-lru-cache", when="^python@:3.2", type=("build", "run")
    )
    depends_on("py-typed-ast@1.3.0:", when="@2.2.5:2.3.0 ^python@3.7.0:3.7")
    depends_on(
        "py-typed-ast@1.4.0:1.4", when="@2.3.1: ^python@:3.7", type=("build", "run")
    )
    depends_on(
        "py-typing-extensions@3.7.4:",
        when="@2.7.3: ^python@:3.7",
        type=("build", "run"),
    )
    depends_on(
        "py-typing-extensions@3.10:", when="@2.8.3: ^python@:3.9", type=("build", "run")
    )
