...
class PyAzuremlPipeline(PythonPackage):
    ...
    depends_on(
        "py-azureml-pipeline-core@1.23.0:1.23", when="@1.23.0", type=("build", "run")
    )
    depends_on(
        "py-azureml-pipeline-steps@1.23.0:1.23", when="@1.23.0", type=("build", "run")
    )

    depends_on(
        "py-azureml-pipeline-core@1.11.0:1.11", when="@1.11.0", type=("build", "run")
    )
    depends_on(
        "py-azureml-pipeline-steps@1.11.0:1.11", when="@1.11.0", type=("build", "run")
    )

    depends_on(
        "py-azureml-pipeline-core@1.8.0:1.8", when="@1.8.0", type=("build", "run")
    )
    depends_on(
        "py-azureml-pipeline-steps@1.8.0:1.8", when="@1.8.0", type=("build", "run")
    )
