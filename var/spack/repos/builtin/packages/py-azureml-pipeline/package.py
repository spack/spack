...
class PyAzuremlPipeline(PythonPackage):
    ...
    depends_on("py-azureml-pipeline-steps@1.23.0:1.23", type=("build", "run"), when="@1.23.0")

    depends_on("py-azureml-pipeline-core@1.11.0:1.11", type=("build", "run"), when="@1.11.0")
    depends_on("py-azureml-pipeline-steps@1.11.0:1.11", type=("build", "run"), when="@1.11.0")

    depends_on("py-azureml-pipeline-core@1.8.0:1.8", type=("build", "run"), when="@1.8.0")
    depends_on("py-azureml-pipeline-steps@1.8.0:1.8", type=("build", "run"), when="@1.8.0")
