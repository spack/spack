...
class PyAzuremlTrainAutomlClient(PythonPackage):
    ...
    depends_on(
        "py-azureml-automl-core@1.23.0:1.23", when="@1.23.0", type=("build", "run")
    )
    depends_on("py-azureml-core@1.23.0:1.23", when="@1.23.0", type=("build", "run"))
    depends_on(
        "py-azureml-dataset-runtime@1.23.0:1.23", when="@1.23.0", type=("build", "run")
    )
    depends_on(
    ...
    depends_on(
        "py-azureml-automl-core@1.11.0:1.11", when="@1.11.0", type=("build", "run")
    )
    depends_on("py-azureml-core@1.11.0:1.11", when="@1.11.0", type=("build", "run"))
    depends_on(
        "py-azureml-dataset-runtime@1.11.0:1.11", when="@1.11.0", type=("build", "run")
    )
    depends_on(
