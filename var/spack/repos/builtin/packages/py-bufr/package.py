...
class PyBufr(CMakePackage, PythonPackage):
    ...
    variant('enable_python', default=False, description='Enable Python interface?')

    depends_on('python@3:', type=('build', 'run'), when='+enable_python')
    depends_on('py-setuptools', type='build', when='+enable_python')
    depends_on('py-numpy', type='build', when='+enable_python')

    def cmake_args(self):
        args = [
            self.define_from_variant('ENABLE_PYTHON')
        ]
    ...
    def install(self, spec, prefix):
        if self.spec.satisfies('+enable_python'):
