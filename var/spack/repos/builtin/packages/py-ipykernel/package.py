...
class PyIpykernel(PythonPackage):
    ...
    depends_on(
        "py-importlib-metadata@:3", when="@6:^python@:3.7", type=("build", "run")
    )
    depends_on(
        "py-importlib-metadata@:4", when="@6.1:^python@:3.7", type=("build", "run")
    )
    depends_on(
        "py-argcomplete@1.12.3:", when="@6.1:^python@:3.7", type=("build", "run")
    )
