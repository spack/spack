...
class PyJaxlib(PythonPackage, CudaPackage):
    ...
    # README.md
    depends_on("cuda@11.8:12", when="@0.4.9:+cuda")
    depends_on("cuda@11.4", when="@0.4:0.4.7+cuda")
    depends_on("cuda@11.1:", when="@0.3+cuda")
    # https://github.com/google/jax/issues/12614
    depends_on("cuda@11.1:11.7.0", when="@0.1+cuda")
    depends_on("cudnn@8.6:", when="@0.4.7:+cuda")
    depends_on("cudnn@8.2", when="@:0.4.7+cuda")
...
""".strip(),
        ...
        elif self.spec.satisfies("@0.4.14:"):
            filter_file(
                "args = parser.parse_args()",
                "args, junk = parser.parse_known_args()",
                "jaxlib/tools/build_wheel.py",
