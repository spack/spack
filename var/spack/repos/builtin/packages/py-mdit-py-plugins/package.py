...
class PyMditPyPlugins(PythonPackage):
    ...
    depends_on("py-flit-core@3.4:3", type=("build", "run"), when="@0.3.1")
    depends_on("python@3.7:", type=("build", "run"), when="@0.3.1")
    depends_on("py-markdown-it-py@1.0:2", type=("build", "run"), when="@0.3.1")

    depends_on("py-setuptools", type="build", when="@:0.2")
    depends_on("py-markdown-it-py@1.0:1", type=("build", "run"), when="@0.2.8")
    depends_on("python@3.6:3", type=("build", "run"), when="@0.2.8")
