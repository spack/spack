...
class PyMelissaCore(PythonPackage):
    ...
    depends_on("py-cloudpickle@2.2.0:", when="@develop,joss", type="run")
    depends_on("py-iterative-stats@0.1.0", when="@develop", type="run")
    depends_on("py-iterative-stats@0.0.4", when="@joss", type="run")
    # requirements_deep_learning.txt (DL with torch)
    depends_on("py-tensorboard@2.10.0:", when="+torch", type="run")
    depends_on("py-matplotlib", when="+torch", type="run")
    depends_on("py-torch@1.12.1:", when="+torch", type="run")
    depends_on("py-python-hostlist", when="@sc23+torch", type="run")
    # requirements_deep_learning.txt  (DL with tensorflow)
    depends_on("py-tensorboard@2.10.0:", when="+tf", type="run")
    depends_on("py-matplotlib", when="+tf", type="run")
    depends_on("py-tensorflow@2.8.0:", when="+tf", type="run")
