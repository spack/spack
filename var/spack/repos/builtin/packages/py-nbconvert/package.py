...
class PyNbconvert(PythonPackage):
    ...
    variant('execute', default=True)
    variant('serve', default=True)
    ...
    depends_on('py-mistune@0.8.1:1.99.99', type=('build', 'run'), when='@5:')
    ...
    depends_on('py-entrypoints', type=('build', 'run'),           when='+serve')
    depends_on('py-entrypoints@0.2.2:', type=('build', 'run'),    when='@5:+serve')
