...
class PyPytest(PythonPackage):
    ...
    depends_on(
        "py-atomicwrites@1.0:", when="@5.3: platform=windows", type=("build", "run")
    )
    ...
    depends_on(
        "py-importlib-metadata@0.12:", when="@5.1: ^python@:3.7", type=("build", "run")
    )
    ...
    depends_on(
        "py-more-itertools@4.0.0:6.0.0",
        when="@4.2.1:4.6.9 ^python@:2",
        type=("build", "run"),
    )
