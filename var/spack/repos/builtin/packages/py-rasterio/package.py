...
class PyRasterio(PythonPackage):
    ...
    # From pyproject.toml
    depends_on("py-setuptools@67.8:", type="build", when="@1.3.9:")
    depends_on("py-cython@0.29.29:", type="build", when="@1.3.3:")
    depends_on("py-cython@0.29.24:0.29", type="build", when="@1.3.0:1.3.2")

    # From setup.py
    depends_on("python@3.8:", type=("build", "link", "run"), when="@1.3:")
    depends_on("python@3.6:3.9", type=("build", "link", "run"), when="@1.2")
    depends_on("python@2.7:2.8,3.5:3.8", type=("build", "link", "run"), when="@1.1")
    depends_on("python@2.7:2.8,3.5:3.7", type=("build", "link", "run"), when="@:1.0")
    depends_on("py-affine", type=("build", "run"))
    depends_on("py-attrs", type=("build", "run"))
    depends_on("py-certifi", type=("build", "run"), when="@1.2:")
    depends_on("py-click@4:", type=("build", "run"), when="@1.2.4:")
    depends_on("py-click@4:7", type=("build", "run"), when="@:1.2.3")
    depends_on("py-cligj@0.5:", type=("build", "run"))
    depends_on("py-numpy@1.18:", type=("build", "link", "run"), when="@1.3:")
