...
class PySfepy(PythonPackage):
    ...
    depends_on("py-petsc4py", when="+petsc", type="run")
    depends_on("py-slepc4py", when="+slepc", type="run")
    depends_on("py-pyamg", when="+pyamg", type="run")
    depends_on("mumps", when="+mumps", type="run")
