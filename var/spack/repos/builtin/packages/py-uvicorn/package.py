...
class PyUvicorn(PythonPackage):
    ...
    with when("+standard"):
        ...
        depends_on("py-uvloop@0.14,0.15.2:", type=("build", "run"), when="platform=linux")
        depends_on("py-uvloop@0.14,0.15.2:", type=("build", "run"), when="platform=darwin")
        depends_on("py-uvloop@0.14,0.15.2:", type=("build", "run"), when="platform=cray")
