...
class Python(AutotoolsPackage):
    ...
    variant(
        "ensurepip",
        default=True,
        description="Build ensurepip module",
        when="@2.7.9:2,3.4:",
    )
    ...
    def filter_compilers(self):
        ...
        filenames = [
            self.get_sysconfigdata_name(),
            self.config_vars["makefile_filename"],
        ]
...
""" % self.print_string(
    ...
    def headers(self):
        ...
        if os.path.exists(config_h):
            ...
            headers = find_headers("pyconfig", self.prefix.include, recursive=True)
            config_h = headers[0]
