...
class Python(AutotoolsPackage):
    ...
    @run_after('install')
    def backup_sysconfigdata(self):
        """Back up the original sysconfigdata file (if it exists) before
        introducing any changes to it."""

        sysconfigdata_name = self.get_sysconfigdata_name()
        if os.path.isfile(sysconfigdata_name):
            # This should be the case when='@2.7.5:2.7.99,3.3.0:'
            sysconfig_dir = self.get_spack_sysconfig_dirname()
            mkdirp(sysconfig_dir)
            copy(self.get_sysconfigdata_name(), sysconfig_dir)
    ...
    def get_spack_sysconfig_dirname(self):
        """Return the full path name of the directory with the backed up
        sysconfigdata file."""

        return join_path(spack.store.layout.metadata_path(self.spec),
                         'spack-sysconfig')
