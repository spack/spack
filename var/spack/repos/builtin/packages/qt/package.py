...
class Qt(Package):
    ...
    version(
        "5.11.3",
        "859417642713cee2493ee3646a7fee782c9f1db39e41d7bb1322bba0c5f0ff4d",
    )
    version(
        "5.11.2",
        "c6104b840b6caee596fa9a35bc5f57f67ed5a99d6a36497b6fe66f990a53ca81",
    )
    ...
    def patch(self):
        ...
        filter_file(
            "^QMAKE_CC .*", "QMAKE_CC = cc", "mkspecs/common/g++-base.conf"
        )
        filter_file(
            "^QMAKE_CXX .*", "QMAKE_CXX = c++", "mkspecs/common/g++-base.conf"
        )
    ...
    def common_config_args(self):
        ...
        if self.spec.variants["freetype"].value == "spack":
            config_args.extend(
                [
                    "-system-freetype",
                    "-I{0}/freetype2".format(
                        self.spec["freetype"].prefix.include
                    ),
                ]
            )
        ...
        if "@5:" in self.spec and MACOS_VERSION:
            config_args.extend(
                ["-no-xinput2", "-no-xcb-xlib", "-no-pulseaudio", "-no-alsa"]
            )
    ...
    def configure(self, spec, prefix):
        ...
        configure(
            "-prefix", prefix, "-v", "-thread", "-shared", "-release", "-fast"
        )
