...
class Rocrand(CMakePackage):
    ...
    def fix_library_locations(self):
        ...
        with working_dir(hiprand_lib_path):
            ...
            for lib in hiprand_libs:
                os.symlink(
                    join_path(hiprand_lib_path, lib), join_path(self.prefix.lib, lib)
                )
        with working_dir(rocrand_lib_path):
            rocrand_libs = glob.glob("*.so*")
            for lib in rocrand_libs:
                os.symlink(
                    join_path(rocrand_lib_path, lib), join_path(self.prefix.lib, lib)
                )
    ...
    def cmake_args(self):
        args = [self.define("BUILD_BENCHMARK", "OFF"), self.define("BUILD_TEST", "OFF")]
