...
#
# Important feature: a version of salome-medcoupling depends on
# a specific version of med package
...
class SalomeMedcoupling(CMakePackage):
    ...
    fields in memory, and use med format for files."""

    maintainers = ["franciskloss", "mthcrts"]
    ...
    depends_on("med@4.1.0+mpi~shared", when="@9.7.0+mpi+static")
    depends_on("med@4.1.0+mpi", when="@9.7.0+mpi")
    depends_on("med@4.1.0~shared", when="@9.7.0~mpi+static")
    depends_on("med@4.1.0", when="@9.7.0~mpi")

    depends_on("salome-configuration@9.6.0", when="@9.6.0")
    depends_on("med@4.1.0+mpi~shared", when="@9.6.0+mpi+static")
    depends_on("med@4.1.0+mpi", when="@9.6.0+mpi")
    depends_on("med@4.1.0~shared", when="@9.6.0~mpi+static")
    depends_on("med@4.1.0", when="@9.6.0~mpi")

    depends_on("salome-configuration@9.5.0", when="@9.5.0")
    depends_on("med@4.1.0+mpi~shared", when="@9.5.0+mpi+static")
    depends_on("med@4.1.0+mpi", when="@9.5.0+mpi")
    depends_on("med@4.1.0~shared", when="@9.5.0~mpi+static")
    depends_on("med@4.1.0", when="@9.5.0~mpi")

    depends_on("salome-configuration@9.4.0", when="@9.4.0")
    depends_on("med@4.0.0+mpi~shared", when="@9.4.0+mpi+static")
    depends_on("med@4.0.0+mpi", when="@9.4.0+mpi")
    depends_on("med@4.0.0~shared", when="@9.4.0~mpi+static")
    depends_on("med@4.0.0", when="@9.4.0~mpi")

    depends_on("salome-configuration@9.3.0", when="@9.3.0")
    depends_on("med@4.0.0+mpi~shared", when="@9.3.0+mpi+static")
    depends_on("med@4.0.0+mpi", when="@9.3.0+mpi")
    depends_on("med@4.0.0~shared", when="@9.3.0~mpi+static")
    depends_on("med@4.0.0", when="@9.3.0~mpi")
    ...
    def setup_build_environment(self, env):
        spec = self.spec
        if "+metis" in spec:
            env.set("METIS_ROOT_DIR", spec["metis"].prefix)

        if "+scotch" in spec:
            name = "PTSCOTCH" if "+mpi" in spec else "SCOTCH"
            env.set(name + "_ROOT_DIR", spec["scotch"].prefix)
            env.set(name + "_INCLUDES_DIR", join_path(spec["scotch"].prefix, "include"))
    ...
    def cmake_args(self):
        ...
        name = "PTSCOTCH" if "+mpi" in spec else "SCOTCH"
        other = "SCOTCH" if name == "PTSCOTCH" else "PTSCOTCH"
        if "+scotch" in spec:
            options.extend(["-DMEDCOUPLING_PARTITIONER_{0}=ON".format(name)])
        else:
            options.extend(["-DMEDCOUPLING_PARTITIONER_{0}=OFF".format(name)])
        options.extend(["-DMEDCOUPLING_PARTITIONER_{0}=OFF".format(other)])
