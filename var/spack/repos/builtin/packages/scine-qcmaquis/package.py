...
class ScineQcmaquis(CMakePackage):
    ...
    variant('blas', values=('openblas','mkl'), default='openblas', description='Which blas library to use.')
    variant('symmetries', default='TwoU1;SU2U1;SU2U1PG;TwoU1PG', description='Which wave functions symmetries to compile (e.g. "TwoU1;SU2U1;SU2U1PG;TwoU1PG")')
    variant('build_tests', default=True, description='Whether to build unit tests using gtest and gmock')
    ...
    def patch(self):
        if self.version <= Version("3.1.2"):
            filter_file("#include <vector>", "#include <vector>\n#include <map>", "dmrg/framework/dmrg/utils/results_collector.h")
