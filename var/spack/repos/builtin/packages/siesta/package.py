...
class Siesta(MakefilePackage, CMakePackage):
    ...
    version("5.0.1",url = "https://gitlab.com/siesta-project/siesta/-/archive/5.0.1/siesta-5.0.1.tar.gz",
            sha256="800a22a831c1d36c6f5fe4aa9c49ee510cbd49a0b2f87b3c8bf3edb6ebd0193a"
    )

    version("5.0.0",url = "https://gitlab.com/siesta-project/siesta/-/archive/rel-5.0/siesta-rel-5.0.tar.gz",
            sha256="a407c31e8fd101efe4bfc68831870c524a8c397a39de7271d31ce0e6a4df92bb"
    )
    version("4.1.5",url = "https://gitlab.com/siesta-project/siesta/-/archive/v4.1.5/siesta-v4.1.5.tar.gz", 
    ...
    variant("cray", default=False, description="Enable specific cray settings for using cray-hdf5 and cray-netcdf modulefiles")
        ...
        if spec.satisfies("@:4.0.2"):
            ...
            if "+cray" in spec :
               configure_args += ["--with-netcdf={0}/lib/libnetcdff.so".format(netcdff_prefix),]
                ...
                configure_args += ["--with-netcdf=%s" % (spec["netcdf-fortran"].libs + spec["netcdf-c"].libs),]
            
            if self.spec.satisfies("%gcc"):
                if "+cray" in spec :
                    configure_args.append("FCFLAGS=-ffree-line-length-0 -I{0}/include".format(netcdff_prefix))
                else:
            ...
            with working_dir("Obj", create=True):
                  ...
                  if spec.satisfies("@:4.1.5%gcc"):
                      ...
                      arch_make.filter("FFLAGS = .*", "FFLAGS = {0}".format('-O2 -fPIC -ftree-vectorize -fallow-argument-mismatch'))
                      ...
                      with open("arch.make", "a") as f:
                            ...
                            if "+netcdf" in spec:
                                if "+cray" in spec :
                                    libs_arg.append("-L{0}/lib -lnetcdff -lnetcdf".format(netcdff_prefix))
                                    libs_arg.append("-L{0}/lib -lhdf5_fortran -lhdf5".format(hdf5_prefix))
                                else:
                                    libs_arg.append("-L{0}/lib -lnetcdff -lnetcdf".format(spec["netcdf-fortran"].prefix))
                                    libs_arg.append("-L{0}/lib -lhdf5_fortran -lhdf5".format(spec["hdf5"].prefix))
                            ...
                            if "elpa" in spec :
                                ...
                                libs_arg.append("-L{0} -lelpa{1}".format(self.spec["elpa"].prefix.lib, elpa_suffix))
                                fppflags_arg.append("-DSIESTA__ELPA  -I{0}".format(join_path(elpa_incdir, "modules")))
                            
                            if "mumps" in spec :
                                libs_arg.append("-L{0} -lmumps_common -lzmumps".format(self.spec["mumps"].prefix.lib))
                                fppflags_arg.append("-DSIESTA__MUMPS ")
                            
                            if "+pexsi" in spec :
                                libs_arg.append("-L{0} -lpexsi_linux".format(self.spec["pexsi"].prefix.lib))
...
class CMakeBuilder(cmake.CMakeBuilder):
    ...
    def cmake_args(self):
        ...
        if "build_type=Debug" in spec:
            args += ["-DFortran_FLAGS=-Og -g -Wall -fcheck=all -fbacktrace -Warray-bounds -Wunused -Wuninitialized"]
        else:
        ...
        if "+mumps" in spec:
            ...
            args += ["-DSIESTA_LINKER_FLAGS=-L{0} -lmumps_common -lzmumps".format(self.spec["mumps"].prefix.lib)]
