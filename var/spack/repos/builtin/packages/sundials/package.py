...
class Sundials(CMakePackage, CudaPackage, ROCmPackage):
    ...
    variant("sycl", default=False, description="Enable SYCL vector")
    ...
    def cmake_args(self):
        ...
        if (spec.satisfies("%apple-clang")) and ("+fcmix" in spec):
            ...
            libgfortran = LibraryList(
                f77("--print-file-name", "libgfortran.a", output=str)
            )
        ...
        # precision
        args.extend(["-DSUNDIALS_PRECISION=%s" % spec.variants["precision"].value])
        ...
        args.extend(["-DUSE_GENERIC_MATH=%s" % on_off("+generic-math")])
        ...
        if "+profiling+caliper" in spec:
            args.extend(
                ["-DENABLE_CALIPER=ON", "-DCALIPER_DIR=%s" % spec["caliper"].prefix]
            )
        ...
        if "+lapack" in spec:
            ...
            args.extend(["-DLAPACK_ENABLE=OFF"])
        ...
        if "+petsc" in spec:
            ...
            args.extend(["-DPETSC_ENABLE=OFF"])

        # Building with RAJA
        if "+raja" in spec:
            args.extend(["-DRAJA_ENABLE=ON", "-DRAJA_DIR=%s" % spec["raja"].prefix])
        else:
            args.extend(["-DRAJA_ENABLE=OFF"])

        # Building with SuperLU_MT
        if "+superlu-mt" in spec:
            if spec.satisfies("@3.0.0:"):
                args.extend(
                    ["-DBLAS_ENABLE=ON", "-DBLAS_LIBRARIES=%s" % spec["blas"].libs]
                )
            args.extend(
                [
                    "-DSUPERLUMT_ENABLE=ON",
                    "-DSUPERLUMT_INCLUDE_DIR=%s" % spec["superlu-mt"].prefix.include,
                    "-DSUPERLUMT_LIBRARY_DIR=%s" % spec["superlu-mt"].prefix.lib,
                ]
            )
            ...
            args.extend(["-DSUPERLUMT_ENABLE=OFF"])

        # Building with SuperLU_DIST
        if "+superlu-dist" in spec:
            args.extend(
                [
                    "-DOPENMP_ENABLE=%s" % on_off("^superlu-dist+openmp"),
                    "-DSUPERLUDIST_ENABLE=ON",
                    "-DSUPERLUDIST_INCLUDE_DIR=%s"
                    % spec["superlu-dist"].prefix.include,
                    "-DSUPERLUDIST_LIBRARY_DIR=%s" % spec["superlu-dist"].prefix.lib,
                    "-DSUPERLUDIST_LIBRARIES=%s" % spec["blas"].libs.joined(";"),
                    "-DSUPERLUDIST_OpenMP=%s" % on_off("^superlu-dist+openmp"),
                ]
            )
        else:
            args.extend(["-DSUPERLUDIST_ENABLE=OFF"])

        # Building with Trilinos
        if "+trilinos" in spec:
            args.extend(
                ["-DTrilinos_ENABLE=ON", "-DTrilinos_DIR=%s" % spec["trilinos"].prefix]
            )
        else:
            args.extend(["-DTrilinos_ENABLE=OFF"])
        ...
        args.extend(["-DEXAMPLES_INSTALL=%s" % on_off("+examples-install")])
    ...
    def test(self):
        ...
        if "+cuda" in self.spec:
            ...
            self.run_test(
                "examples/nvector/cuda/test_nvector_cuda",
                options=["10", "0", "0"],
                work_dir=self._extra_tests_path,
            )
        if "+rocm" in self.spec:
            ...
            self.run_test(
                "examples/nvector/hip/test_nvector_hip",
                options=["10", "0", "0"],
                work_dir=self._extra_tests_path,
            )
        if "+sycl" in self.spec:
            ...
            self.run_test(
                "examples/nvector/sycl/test_nvector_sycl",
                options=["10", "0", "0"],
                work_dir=self._extra_tests_path,
            )
        return
