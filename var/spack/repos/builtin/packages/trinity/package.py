...
class Trinity(MakefilePackage):
    ...
    def install(self, spec, prefix):
        ...
        force_remove(join_path(prefix, '.gitmodules'))
        force_remove(join_path(prefix, 'Butterfly', '.err'))
        force_remove(join_path(prefix, 'Butterfly', 'src', '.classpath'))
        force_remove(join_path(prefix, 'Butterfly', 'src', '.err'))
        force_remove(join_path(prefix, 'Butterfly', 'src', '.project'))
        remove_linked_tree(join_path(prefix, 'Butterfly', 'src', '.settings'))
        remove_linked_tree(join_path(prefix, 'Inchworm', 'src', '.deps'))
        remove_linked_tree(join_path(prefix, 'trinity-plugins', 'ParaFly-0.1.0', 'src', '.deps'))
        force_remove(join_path(prefix, 'trinity-plugins', 'seqtk-trinity-0.0.2', '.gitignore'))
        force_remove(join_path(prefix, 'trinity-plugins', 'slclust' ,'bin', '.hidden'))

    def setup_environment(self, spack_env, run_env):
        run_env.set('TRINITY_HOME', self.prefix)
