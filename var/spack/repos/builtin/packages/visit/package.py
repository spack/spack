...
class Visit(CMakePackage):
    ...
    version(
        "2.13.3",
        sha256="cf0b3d2e39e1cd102dd886d3ef6da892733445e362fc28f24d9682012cccf2e5",
    )
    version(
        "2.13.0",
        sha256="716644b8e78a00ff82691619d4d1e7a914965b6535884890b667b97ba08d6a0f",
    )
    version(
        "2.12.3",
        sha256="2dd351a291ee3e79926bc00391ca89b202cfa4751331b0fdee1b960c7922161f",
    )
    version(
        "2.12.2",
        sha256="55897d656ac2ea4eb87a30118b2e3963d6c8a391dda0790268426a73e4b06943",
    )
    version(
        "2.10.3",
        sha256="05018215c4727eb42d47bb5cc4ff937b2a2ccaca90d141bc7fa426a0843a5dbc",
    )
    version(
        "2.10.2",
        sha256="89ecdfaf197ef431685e31b75628774deb6cd75d3e332ef26505774403e8beff",
    )
    version(
        "2.10.1",
        sha256="6b53dea89a241fd03300a7a3a50c0f773e2fb8458cd3ad06816e9bd2f0337cd8",
    )
    ...
    # system X libraries and include files installed.
    depends_on(
        "vtk@8.1.0+opengl2+osmesa~python",
        patches=[
            patch("vtk_compiler_visibility.patch"),
    ...
    def cmake_args(self):
        ...
        if "^mesa" in spec:
            args.append("-DVISIT_LLVM_DIR:PATH={0}".format(spec["llvm"].prefix))
            args.append("-DVISIT_MESAGL_DIR:PATH={0}".format(spec["mesa"].prefix))

        if "+hdf5" in spec:
            args.append("-DVISIT_HDF5_DIR:PATH={0}".format(spec["hdf5"].prefix))
            if "+mpi" in spec:
                args.append("-DVISIT_HDF5_MPI_DIR:PATH={0}".format(spec["hdf5"].prefix))

        if "+silo" in spec:
            args.append("-DVISIT_SILO_DIR:PATH={0}".format(spec["silo"].prefix))
