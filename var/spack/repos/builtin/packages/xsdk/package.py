...
class Xsdk(BundlePackage, CudaPackage, ROCmPackage):
    ...
    variant(
        "datatransferkit",
        default=True,
        description="Enable datatransferkit package build",
    )
    ...
    variant(
        "libensemble", default=True, description="Enable py-libensemble package build"
    )
    variant(
        "precice",
    ...
    variant(
        "butterflypack", default=True, description="Enable butterflypack package build"
    )
    ...
    xsdk_depends_on(
        "hypre@develop+superlu-dist+shared", when="@develop", cuda_var="cuda"
    )
    ...
    xsdk_depends_on(
        "mfem@4.0.1-xsdk+mpi~superlu-dist+petsc+sundials+examples+miniapps",
        when="@0.5.0",
    )
    xsdk_depends_on(
        "mfem@3.4.0+mpi+superlu-dist+petsc+sundials+examples+miniapps", when="@0.4.0"
    )
    xsdk_depends_on(
        "mfem@3.3.2+mpi+superlu-dist+petsc+sundials+examples+miniapps", when="@0.3.0"
    )
    ...
    xsdk_depends_on(
        "datatransferkit@master", when="@develop +trilinos +datatransferkit"
    )
    dtk7ver = "3.1-rc2" if sys.platform == "darwin" else "3.1-rc3"
    xsdk_depends_on(
        "datatransferkit@" + dtk7ver, when="@0.7.0 +trilinos +datatransferkit"
    )
    ...
    xsdk_depends_on(
        "petsc@3.12.1+mpi+hypre+superlu-dist+metis+hdf5~mumps+double~int64",
        when="@0.5.0",
    )
    xsdk_depends_on(
        "petsc@3.10.3+mpi+hypre+superlu-dist+metis+hdf5~mumps+double~int64",
        when="@0.4.0",
    )
    xsdk_depends_on(
        "petsc@3.8.2+mpi+hypre+superlu-dist+metis+hdf5~mumps+double~int64",
        when="@0.3.0",
    )
    ...
    xsdk_depends_on(
        "amrex@21.10+sundials", when="@0.7.0 %intel", cuda_var="cuda", rocm_var="rocm"
    )
    xsdk_depends_on(
        "amrex@21.10+sundials", when="@0.7.0 %gcc", cuda_var="cuda", rocm_var="rocm"
    )
    xsdk_depends_on(
        "amrex@21.10+sundials", when="@0.7.0 %cce", cuda_var="cuda", rocm_var="rocm"
    )
    ...
    xsdk_depends_on(
        "tasmanian@6.0+xsdkflags+blas~openmp",
        when="@0.4.0",
        cuda_var=["cuda", "?magma"],
    )
    ...
    xsdk_depends_on(
        "phist@develop ~fortran ~scamac ~openmp ~host ~int64", when="@develop +phist"
    )
    xsdk_depends_on(
        "phist@1.9.5 ~fortran ~scamac ~openmp ~host ~int64", when="@0.7.0 +phist"
    )
    xsdk_depends_on(
        "phist@1.9.3 ~fortran ~scamac ~openmp ~host ~int64", when="@0.6.0 +phist"
    )
    xsdk_depends_on(
        "phist@1.8.0 ~fortran ~scamac ~openmp ~host ~int64", when="@0.5.0 +phist"
    )
    xsdk_depends_on(
        "phist@1.7.5 ~fortran ~scamac ~openmp ~host ~int64", when="@0.4.0 +phist"
    )

    xsdk_depends_on(
        "ginkgo@develop ~openmp",
        when="@develop +ginkgo",
        cuda_var="cuda",
        rocm_var="rocm",
    )
    ...
    xsdk_depends_on(
        "heffte@2.0.0+fftw", when="@0.6.0 +heffte", cuda_var=["cuda", "?magma"]
    )
