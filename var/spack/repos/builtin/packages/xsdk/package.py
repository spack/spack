...
class Xsdk(Package):
    ...
    version('0.5.0', 'a52dc710c744afa0b71429b8ec9425bc')
    ...
    depends_on('mfem@develop+mpi+hypre+superlu-dist+petsc~sundials+examples+miniapps', when='@0.5.0')
    ...
    depends_on('petsc@develop+trilinos+mpi+hypre+superlu-dist+metis+hdf5~mumps+double~int64',
               when='@develop')
    depends_on('petsc@3.12.0+mpi+trilinos+hypre~superlu-dist+metis+hdf5~mumps+double~int64',
               when='@0.5.0')
    ...
    depends_on('sundials@4.1.0~int64+hypre', when='@0.5.0')
