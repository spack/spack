...
class Xyce(CMakePackage):
    ...
    depends_on('trilinos~adios2~boost~cgns~chaco~cuda~cuda_rdc~debug~dtk~exodus~float~hypre~ifpack2~intrepid~intrepid2~ipo~matio~mesquite~metis~minitensor~ml~muelu~mumps~netcdf~openmp~phalanx~piro~pnetcdf~python~rol~rythmos~shards~shared~shylu~stk~stratimikos~strumpack~superlu~superlu-dist~teko~tempus~wrapper~zlib~zoltan2')
